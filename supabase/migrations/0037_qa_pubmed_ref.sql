-- qas 테이블에 PubMed 참고 논문 정보 (단일 reference 객체) 추가
-- Phase 6: AI 인용 친화 + E-E-A-T 강화. 카드 하단 ref. 박스 노출용.
-- 구조: { pmid, doi, title, journal, year, authors_short, pubmed_url, doi_url, reasoning }
-- reasoning은 운영 검수용 내부 메타 (UI 노출은 운영자 화면에서만).

alter table public.qas
  add column if not exists pubmed_ref jsonb;

comment on column public.qas.pubmed_ref is
  '단일 PubMed reference 객체. 키: pmid, doi, title, journal, year, authors_short, pubmed_url, doi_url, reasoning(내부). null이면 미매칭.';

-- search_qas_scored RPC 갱신 — pubmed_ref 반환 추가
