-- 0229: create/update_procedure_review — 부모 앵커도 lazy 보장 (작업 D-c)
--
-- live(2026-06-04) 정의 VERBATIM + 앵커 lazy INSERT 의 대상 ko 를 "자기 + 부모" 로 확장.
--   기존: WHERE t.ko = <procedure_ko>
--   변경: WHERE t.ko IN (<procedure_ko>, (<procedure_ko> 의 parent_ko))
--   → 자식 후기 발행 시 자식 앵커 + 부모 앵커(있으면) 동시 lazy 생성(앵커 기준 자기≥1 → family≥1).
-- 앵커는 기존과 동일 'draft'. ON CONFLICT 멱등. 나머지 로직·시그니처 불변.
...
BEGIN
  ...
  -- [C1 + D] 발행이면 자기 + 부모 시술 앵커(review_summary) lazy 생성(없을 때만, 멱등). 앵커는 draft.
...
BEGIN
  ...
  -- [C1 + D] published 면 자기 + 부모 앵커 lazy 생성(없을 때만, 멱등). 앵커는 draft.
  IF p_status = 'published' THEN
