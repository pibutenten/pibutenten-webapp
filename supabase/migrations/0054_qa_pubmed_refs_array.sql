-- =============================================================
-- 0054. qas.pubmed_refs jsonb[] — 참고문헌 멀티 ref
--
-- 기존 pubmed_ref(jsonb, 단일 객체)는 그대로 유지 (호환성).
-- 새 컬럼 pubmed_refs(jsonb[])로 0~N개의 참고문헌을 보관.
-- 기존 데이터는 단일 ref가 있으면 1개짜리 배열로 자동 백필.
-- =============================================================

-- 1. 컬럼 추가
alter table public.qas
  add column if not exists pubmed_refs jsonb[];

comment on column public.qas.pubmed_refs is
  'PubMed reference 배열. 각 원소는 {pmid, doi, title, journal, year, authors_short, pubmed_url, doi_url, reasoning?}. 빈 배열 또는 NULL이면 참고문헌 없음.';

-- 2. 백필: 기존 단일 pubmed_ref → 1개짜리 배열
update public.qas
   set pubmed_refs = array[pubmed_ref]
 where pubmed_ref is not null
   and (pubmed_refs is null or array_length(pubmed_refs, 1) is null);

-- 3. search_qas_scored RPC 갱신 — pubmed_refs도 함께 반환
...
-- 검증
select count(*) as total_qas,
       count(*) filter (where pubmed_refs is not null) as has_refs,
       count(*) filter (where pubmed_ref is not null) as has_single_ref
from public.qas;
