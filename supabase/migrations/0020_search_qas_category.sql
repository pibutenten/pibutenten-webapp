-- search_qas_scored RPC에 category·hide_doctor_credential 컬럼 추가.
-- 카드(QACard)에서 카테고리·credential 분기에 필요.
...
begin
  ...
  select s.id, s.question, s.answer, s.meta, s.keywords, s.like_count, s.view_count,
    ...
    case when p.id is null then null::jsonb else jsonb_build_object('id', p.id, 'display_name', p.display_name, 'avatar_url', p.avatar_url, 'alt_display_name', p.alt_display_name, 'alt_avatar_url', p.alt_avatar_url) end as author,
    s.type_text as type,
    ...
    s.category, s.hide_doctor_credential,
