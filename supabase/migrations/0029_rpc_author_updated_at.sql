-- search_qas_scored RPC 갱신: author jsonb에 updated_at 추가.
-- 카드는 avatar src에 ?v={updated_at hash}를 붙여 브라우저·CDN 캐시 무효화.
-- profile.avatar_url이 변경되면 updated_at이 갱신되므로 자동으로 새 URL.
