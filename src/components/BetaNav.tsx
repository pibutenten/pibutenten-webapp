 ...
 * 레이아웃(1080 컨테이너·footer·카드·피드)은 기존 그대로 두고, 내비만 새 5탭 구조로 교체.
...
const ICON = {
  ...
  search: <I d={<><circle cx="11" cy="11" r="7" /><path d="m21 21-4.3-4.3" /></>} size={20} />,
};

type Tab = { href: string; label: string; icon: React.ReactNode; match: (p: string) => boolean };
const TABS: Tab[] = [
  { href: "/beta/record", label: "시술 기록", icon: ICON.book, match: (p) => p.startsWith("/beta/record") },
    ...
    <>
      ...
      <header className="sticky top-0 z-50 backdrop-blur" style={{ background: "rgba(255,255,255,0.92)" }}>
        <div className="mx-auto flex w-full max-w-[1080px] items-center gap-4 px-4 py-3 sm:px-6">
          ...
          <Link href="/search" aria-label="검색" title="검색" className="flex min-h-[44px] items-center rounded-md p-3 text-[var(--text)] sm:min-h-0 sm:p-2">{ICON.search}</Link>
