...
/**
 * InfoBetaShell — 신뢰·법적·안내 페이지(/about·/terms·/privacy·/contact·/disclaimer·
 *   /editorial-policy·/medical-review·/corrections·/disclosures·/doctor-guidelines)
 *   본문을 신규 스킨(베타 셸)로 감싸는 공용 클라이언트 wrapper.
 *
 ...
 *   - 검색 제출은 운영 홈(/?q=)으로 라우팅(useBetaSearchRouting, 운영 정합).
 *
 * 격리: beta-skin.module.css / GlobalChrome / layout.tsx / robots.ts 무수정.
...
export default function InfoBetaShell({ children }: { children: ReactNode }) {
  const search = useBetaSearchRouting();
  return (
    <BetaSkinShell active="마이" wide back={false} {...search}>
      {children}
    </BetaSkinShell>
  );
}
