...
/**
 * WriteEditShell — 글 수정(/write/{shortcode}) 본문을 신규 스킨(앱 셸)로 감싸는 얇은 클라이언트 wrapper.
 *
 * 선례: src/components/info/InfoShell.tsx (server page = 권한·데이터 / client wrapper = 셸+본문).
 ...
 *   - 검색 제출은 운영 홈(/?q=)으로 라우팅(useSearchRouting, WriteView 와 동일 정합).
 *   - back={false} — 수정 본문이 자체 BackButton 을 이미 렌더하므로 중복 방지(InfoShell 선례).
 ...
 *
 * 격리: AppShell / GlobalChrome / write/layout.tsx(noindex) 무수정.
 */
...
export default function WriteEditShell({ children }: { children: ReactNode }) {
  ...
  return (
    <AppShell active="글쓰기" back={false} {...search}>
      {children}
    </AppShell>
  );
}
