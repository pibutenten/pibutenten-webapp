...
export async function generateMetadata({
  ...
  // /old-skin 은 박제 백업이므로 검색 분기도 noindex,follow 유지(index 금지).
  if (query) {
    return {
      title: { absolute: `[구버전] '${query}' 검색 결과 | 피부텐텐` },
  ...
  const d = count ?? 0;
  const title = "[구버전] 피부텐텐 | 피부가 예뻐지는 모든 이야기";
  const description = `피부과 전문의 ${d}명이 리프팅·스킨부스터·안티에이징 시술 질문에 직접 답합니다. 시술별 후기 집계까지.`;
  return {
    title: { absolute: title },
    description,
    // /old-skin 전 페이지 noindex,nofollow 강제(박제 백업, 색인 금지).
    robots: { index: false, follow: false },
    alternates: { canonical: `${SITE_URL}/` },
    openGraph: {
      title,
