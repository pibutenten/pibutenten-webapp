...
/** 지표별 설명 — 동봉 기술문서(피부 관점) 기반. 행동 권고만(의료법 §56 톤). */
const KPI_INFO: Record<string, { label: string; color: string; text: string }> = {
  tanning: {
    label: "UVB 홍반",
    color: "#E0382E",
    text: "햇볕에 타고 붉어지게(홍반·화상) 만드는 자외선이에요. 국제표준 UV Index(0~11+)로 표시하며, 높을수록 그늘·모자·선크림이 필요해요. 주로 한낮에 가장 강해집니다.",
  },
  aging: {
    label: "UVA 노화",
    color: "#B45CB0",
    text: "색소·주름·탄력 저하를 부르는 ‘노화 자외선’이에요. 흐린 날·겨울·실내 창가에서도 깔려 있어, 매일 자외선 차단이 중요해요. (전천일사량에서 환산한 지수)",
  },
  pm: {
    label: "미세먼지",
    color: "#C9A21E",
    text: "PM2.5·PM10 중 더 나쁜 등급으로 표시해요. 나쁜 날엔 외출 후 평소보다 꼼꼼히 세안하고 보습으로 피부 장벽을 지켜주세요.",
  },
  block: {
    label: "구름투과",
    color: "#3C8CC8",
    text: "구름을 통과해 피부에 닿는 자외선 비율이에요. 완전히 흐린 날에도 자외선의 절반 이상이 통과해요 — 흐려도 차단은 필수예요. (서울 실측 기반)",
  },
};
