 ...
 *
 * 렌더: 베타 스킨 셸(BetaAdminAuthErrorsView)로 위임. 데이터 fetch·가드는 이 서버 페이지가 그대로 담당.
 *   - 상단바·배경만 베타 셸, 본문(기간 카운트 + 최근 50건 표)은 운영 구조 유지.
 *
 * UI 최소 구성 (1차):
...
import BetaAdminAuthErrorsView, {
  type BetaAuthErrorRow,
} from "./BetaAdminAuthErrorsView";
...
export default async function AuthErrorsPage() {
  ...
  const rows = (rowsData ?? []) as BetaAuthErrorRow[];
