...
/**
 * AdminCardEditView — /admin/cards/[id]/edit "카드 편집" 셸 래퍼 (클라이언트).
 *
 * 원칙(승격·단일화): UI 는 앱 스킨 톤(AppShell), 편집 폼 로직·검증·저장 액션은
 ...
 *   - 이 컴포넌트는 AppShell + useSearchRouting 안에 운영 EditClient 를 그대로 임베드한다.
 *   - EditClient 가 폼·저장·슬러그·복구 흐름을 자체 처리하므로 로직 재구현 없이 그대로 사용.
 *
 * 격리: 운영 EditClient 무수정. 앱 셸만 입힌다. props 계약은 EditClient 의 것을 그대로 따른다
...
export type AdminCardEditViewProps = ComponentProps<typeof EditClient>;

export default function AdminCardEditView(
  props: AdminCardEditViewProps,
) {
  const search = useSearchRouting();

  return (
    <AppShell active="마이" wide back="/admin/cards" {...search}>
