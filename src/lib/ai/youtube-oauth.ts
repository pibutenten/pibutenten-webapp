 ...
 *
 * 작동 조건:
 *   .env.local에 YOUTUBE_OAUTH_CLIENT_ID / _SECRET / _REFRESH_TOKEN 모두 있을 때만 활성.
 *   하나라도 비어있으면 isOauthAvailable() = false → 호출자는 watch-page 등 fallback로.
 *
 * 흐름:
 *   1) refresh_token → access_token (https://oauth2.googleapis.com/token, in-memory cache)
...
export async function fetchCaptionsViaOauth(
  ...
  if (!isOauthAvailable()) return null;
  try {
