...
### Changed
- **피부톤(Fitzpatrick) 질문 UI 전면 교체** (`OnboardingClient.tsx`) — 질문을 "내 피부색과 가장 가까운 얼굴을 골라주세요."로, 긴 반응 설명문 6줄(2줄×6종·인종 표기) 리스트 → **코드 SVG 얼굴 아이콘 6개(3×2 그리드) + 짧은 캡션**("북유럽·켈트계 매우 흰 피부"~"아프리카계 짙은 갈색 피부"). 이미지 에셋 0(로딩 무부담). 톤·캡션은 신규 SSOT `lib/fitzpatrick.ts`(원장 확정 6색 #FDF0E6/#F4D8C3/#E6C3A1/#BE9366/#8B5A33/#422818). 저장값(1~6)·전파 RPC 무변.
