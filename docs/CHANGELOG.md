...
## [2026-07-18] — 원장 대표 논문 제목·저널·연도 표시 (pmids → papers 승격)
...
- code-reviewer 검수 **[치명] 1건 반영**(PostDetail 미러에 papers 누락 → 정합 복원) + [경고] 반영(저널·연도 인라인 `display:inline` — `.profileRow dd span{display:block}` 자손 선택자 회피). `tsc --noEmit`·`npm run build` 통과.
...
- **동기화 규칙 갱신**: 루트 `CLAUDE.md` §5 동기화 표에 `step1_v5.md`의 `{{PUBMED_PROCEDURE_DICT}}` placeholder ↔ `step1.ts` 동일 리터럴 dual-SSOT 페어 추가(placeholder 이름 변경 시 양쪽 동시 갱신 — 한쪽만 바꾸면 사전이 조용히 빠짐).
- **설계 결정 재확인**(원장 확정): ① PubMed 검색어는 하이브리드 유지(번역=DB 사전 SSOT, AI=질문별 초점 보강) — DB 사전만 쓰면 질문별 정밀도 하락이라 AI 검색어 제거 안 함. ② 글 날짜는 현행 유지(DB 3개 저장[created/updated/reviewed]·화면 1개[검수일] 표시, 정책 문서만 4-date 개념 서술 — 팩트체크일 컬럼 미존재).
...
### Changed
- **(R5) 리포트·마이페이지 디자인 보정 33항목** (디자이너 2차 피드백 — 계획 SSOT `docs/plans/260709 리포트·마이페이지 디자인 보정 계획 (R5).md`): ①리포트 1뎁스 — 접힘 통증·만족도 20% 확대+막대 하단 정렬·96% SemiBold·%↔라벨 간격 반절, 펼침 마커 번개 광학 보정·효과↔시점 간격 2/3 ②2뎁스 — 통증 게이지 오버레이 채움(R4 누락분)·만족도 분포 숫자 좌측정렬+바 연장·4.9 축소·/5.0 #B7C1C6·"할까요?" 카피·타임라인 축선 원 뒤로+통계 구분선 제거·저장/공유 아이콘 #A9AEBB·후기 유도 카드 10% 확대·**다른 시술 순위 radial-gradient 배경+흰 텍스트로 전면 반전** ③헤더·캔버스 #F5FBFF 동색화(리포트·마이·내정보 3화면, 데스크탑 포함 — /my/settings 동반 파급 의도 수용) ④/my — **모바일 헤더 승격**(로고 헤더 제거→마이페이지+벨+설정, AppShell titleHeader 신설)·정보 칩 닉네임 아래·r=8·새 하트/북마크 아이콘 #383F47·닉네임 18px·피부정보 접힘 여백 축소 ⑤내 정보(/[handle] 본인) — **backHeader title/action 슬롯 확장**(`[< 내정보 … 수정]`)·통계 20px/#676B76·라벨 15px/#9DA1AA·글 목록 그림자 제거·**필터 칩 풀블리드**(내정보만). 링크(a) 글자색도 무계층 reset 에 덮이는 함정 확인 — 리포트 더보기·Q&A 보러가기 인라인 style 화. TT symbol.svg 는 자산 미수령으로 대기.
...
### Added
- **유입 분석(Acquisition) 대시보드** (0410c06, 원장 요청 — 구 리서치 패널 대체): "어디서·어떻게 들어왔는지" admin 자체 집계. 마이그 0357 `traffic_landings`(채널·referrer·UTM·기기/OS·인앱·지역, IP 원본 미저장) + `get_traffic_overview` RPC(admin 가드). 수집=클라 비컨 `LandingTracker`(세션 1회) → `POST /api/landing`(채널 분류·UA/지오 파싱·IP rate-limit·best-effort). admin `TrafficPanel`(기간 토글·채널 막대·상위 유입처/랜딩·기기/OS·캠페인). **오가닉 검색어는 검색엔진 정책상 자체 수집 불가 → 구글 서치콘솔·네이버 서치어드바이저 등록 필요**(위젯에 안내 링크). supabase-specialist·code-reviewer 치명 0.

### Changed
- **유입 분석 — 앱 채널 분리 + 모바일 반응형**: LandingTracker 가 `Capacitor.isNativePlatform()` 로 네이티브 앱 진입을 감지해 `isApp` 전달 → 라우트가 referrer/UTM 무관하게 **"앱" 채널**로 분리 집계. TrafficPanel 그리드(상위 유입처/랜딩 2단·기기/OS/인앱 3단)를 `auto-fit minmax` 로 바꿔 모바일에서 1단 스택(반응형).
- **관리자 대시보드 활동 통계 숫자 모바일 반응형** (0410c06): ActivityKpis 숫자 `text-xl` 고정 → `clamp(14px,4vw,20px)`(데스크탑 불변) — 6자리 값(예: 조회수 33,012)이 모바일 4열에서 칸을 넘던 것 보정(운영 통계 카드와 동일 방식, fa95c7f 후속·누락분).
- **앱 배경 단색 #F5FBFF 기본 통일 (투데이만 그라데이션)** (7da5b66, 원장 확정): 기본(.root) 캔버스를 브랜드 그라데이션 → 단색 #F5FBFF 로 뒤집음. `.canvasGradient` variant(그라데이션+헤더 유리 질감+칩바 틴트 복원)를 신설해 **투데이(/today)만** opt-in(AppShell canvas="gradient"). 피드·리포트·마이·내정보·내노트·대시보드(admin/clinic/doctor 각 계정 전부) 전부 단색. base `.header`/`.chipBar`/데스크탑 헤더 단색화(구 canvasReport/My/Profile .header·chipBar 오버라이드 삭제 — 기본과 동값). 초기 로딩 FOUC 제거: loading.tsx 6개(투데이 제외) 하드코딩 그라데이션 → #F5FBFF(첫 페인트부터 목적 페이지와 동색). code-reviewer 치명 0.
- **원장 페이지 헤더 '< 뒤로  ○○ 원장님의 답변 N편' 한 줄** (f1d9450, 원장 요청): backHeader.title 타입 string→ReactNode + 데스크탑 백로우(.backRowDesktop)에 title 렌더(.backTitle 18px/800·파란 b). DoctorProfileView 본문 답변 헤더 2개(데스크탑·모바일) 제거·헤더로 이동, SEO 계층은 sr-only h2 보존.
- **피드 상단 소개 글상자를 투데이 그라데이션 CTA(→/app)로** (b56cc35, 원장 요청): HomeBrandIntro 흰 카드 → greetCard 동일 그라데이션(#1e9fe0→#8fe0ff)+흰 글씨(위 2줄 굵게·3줄째 같은 크기 약한 흰색), 글상자 전체 /app 링크. 비로그인·크롤러 노출 규칙·SEO 텍스트 유지.
- **리포트 정렬 칩을 피드처럼 헤더 칩바 슬롯으로 이전** (3d625aa, 원장 제보 — "리포트 최상단도 피드처럼 두 층이 붙어 함께 움직이게"): 허브 정렬 칩을 본문 sticky → **셸 헤더 칩바 슬롯**(AppShell chips=.topStack)으로 이전해 헤더와 한 요소로 이동/숨김(모바일 동시 숨김·데스크탑 상시 고정, 피드와 동일 경험). 정렬 상태를 ReportsShell 로 리프트(ReportsSortContext, 기존 category 패턴)·본문(ReportsIndexView)은 구독만·뒤로가기 스냅샷 복원 유지. 칩은 피드처럼 버튼 배열 그대로 전달(group 래퍼 없이 aria-pressed). 상세 후기 정렬 칩은 헤더에 못 올리므로(최상단=히어로) 본문 sticky 유지하되 z-30·데스크탑 top 72px 구간 스코프로 정리(**섹션 제목은 고정 제외 — 일반 흐름, 정렬 칩만 sticky**, cab011e 원장 확정). `.canvasReport .chipBar` 배경 #F5FBFF 동색. 문서: PRD "칩바=헤더 한 덩어리" 표준으로 정정.

### Fixed
- **전역 sticky 계층 전수 점검 + safe-area 예방 2건** (391b3d5): facc6eb 표준 기준으로 src 전체(sticky/fixed/z 전수 grep)를 점검 — 본문 sticky 헤더 침범 위반 0건(리포트 2곳 외 본문 sticky 바 부재, z≥40 은 전부 의도적 오버레이). 부수 발견인 `OfflineBanner`·임시저장 테이크오버 헤더의 네이티브 상태바 겹침만 `--sat` 보정(웹 무영향).
- **리포트 정렬 칩이 헤더를 침범하던 계층 버그** (원장 데스크탑 캡처 제보): 허브·상세 정렬 칩이 `z-41`(헤더 .topStack=z-40 위)로 선언돼 ①데스크탑(헤더 상시 고정)에서 칩이 로고·검색창 위로 겹치고 ②모바일에서 복귀 헤더를 칩이 덮었음. 표준 정립 — 모바일=헤더+칩 동시 숨김/복귀(현행)·복귀 헤더가 칩 위를 덮음(z-30), 데스크탑=상시 고정 헤더 "아래"(top 72px)에 sticky(허브=상시·상세=후기 구간 스코프, 피드 칩바와 일관). 정렬 변경 자동 스크롤 여백 데스크탑 분기(scroll-mt 128px) 동반.
...
### Added
- **관리자 카드 공용화 (SSOT, C10)** — `AdminView` 의 `Stat`·`Tool` 카드를 `components/skin/OpsCards.tsx` 로 순수 추출, admin·clinic 공유(디자인 드리프트 0). `/clinic` 대시보드를 admin 톤(`--ink`/`--tt-blue`/`--line`)으로 재정렬(환자 관리 active·시술기록 관리 S4까지 준비중).
- **환자 관리 테이블 `/clinic/patients`** — 관리자 회원관리(/admin/users)식 정렬·검색·필터 표: 컬럼(이름·아이디·생년월일·나이·성별·등록번호·상태·등록일·최근시술일·시술수), 헤더 클릭 정렬(6종, URL 동기)·상태 필터 칩·통합 검색(이름·등록번호·아이디·**생일 790126** — 클라 파싱 후 p_search OR p_birthdate)·반응형 표/카드·"더 보기". `api/clinic/patients` v2 파라미터 배선(서버 재검증).
- **생년월일 직접입력 (C12)** — `BirthdateSelect` 에 텍스트 타이핑(790126·19790126·1979-01-26 유연 파싱·2자리연도 피벗) + 드롭다운 병행(양방향 동기). 온보딩 회귀 없음(무효입력 value="").
- **DB: `get_clinic_patients` 검색 OR (마이그 0352)** — 한 검색창에서 이름·등록번호·아이디·생일 OR 매칭(0351 의 AND 오류 정정).

### Changed
- **환자 등록 개선** — 안내 "입력한 회원에게 동의 요청 알림이 발송돼요."(‘물어보세요’ 제거), 중복 등록(409) 시 "이미 등록된 환자예요"+목록 링크(재요청 안 함, C8), 매칭 실패(400) 시 "회원이 생일 수정 후 재등록" 안내(C11), admin 톤.
- **회원 동의화면 재설계 `/onboarding/clinic-link/[id]`** — 제목 단축("병원 연결 동의")·**본인 확인(병원이 등록한 이름) 최상단 강조**·제공 정보 칩·이중 동의 고지·**버튼 수직 위계**(동의 풀폭 primary + 거절 약한 텍스트). phase·데이터 로드 로직 불변, form 블록만 재배치.
- **환자 상세 admin 톤 + 생일·나이·성별** — `get_clinic_patient`(단건)는 age_years 미반환이라 상세는 생일에서 만나이 직접 계산(총괄 검수 정정).

---

## [2026-07-06] — 병원 운영 프로그램 S1: 시술기록 관리 DB 토대 (마이그 0350·0351)

계획 SSOT: `docs/plans/260706 병원 운영 프로그램 (환자·시술기록 관리) 설계.md`. 원장 지시로 /clinic 을 관리자 페이지급 운영 프로그램(환자 관리 + 시술기록 관리)으로 재설계 — 그 첫 단계(DB). 프로세스: 4인 브레인스토밍(DB·목록·상세·IA) → 디자인 서브에이전트(동의화면) → 3인 DB 검토(치명 4 사전 차단) → 마이그 위임 작성 → 총괄 검수(정정 1: clinic_delete_visit revoked 차단) → 디비전문가 독립 재검수(치명 0) → UTF-8 적용·검증.

### Added
- **병원 시술기록 관리 RPC 5종 (마이그 0350)** — 병원이 자기 대행 노트를 **조회·수정·삭제**(기존엔 쓰기만 가능·읽기 불가였음): `get_clinic_patient_visits`(환자별 타임라인) · `get_clinic_visits`(지점 전체 대장, 날짜범위·정렬·검색) · `get_clinic_calendar_summary`(캘린더 월 집계) · `clinic_update_visit` · `clinic_delete_visit`. **3중 소유 경계**(source='clinic'·자기 지점·연결 회원) + active 연결만 수정/삭제(C2 revoked 조회만) + 후기 달린 노트 차단(C5) + 불변컬럼 보호 + 정렬 SQL 인젝션 차단. 인덱스 2종(대장 정렬·생일 검색).
- **`get_clinic_patients` v2 (마이그 0351)** — 정렬(이름 ko-icu·생일·나이·최근시술·시술수·상태)·상태필터·생일 동등검색·페이지네이션 + `last_visit_on`·`visit_count`·`age_years` 집계. RETURNS TABLE 컬럼 추가라 DROP 후 재생성(기존 프론트 호출 무영향 — 기존 13컬럼 순서 보존).
...
### 알려진 트레이드오프 / 백로그
- **sitemap.xml 캐싱 헤더(경미, 현행 유지)**: `revalidate=3600` ISR 로 프레임워크 레벨 캐싱은 동작하나 HTTP `Cache-Control` 은 Next 메타 라우트 기본값(`max-age=0`)이라 RSS(라우트 핸들러, `s-maxage`)와 다름. 기능상 무해(크롤러는 ISR 캐시본 수신)라 라우트 핸들러 전환의 위험 대비 이득이 낮아 현행 유지 — 필요 시 별도 전환.
- **A 엣지케이스(경미)**: 시술후기에서 시술명 미선택 + 본문만 입력한 상태의 탭 전환은 경고 없이 유실(제출 불가 미완성 상태라 영향 작음 — 재검수도 '경미' 분류).
- **"(수정됨)" 표시(제품 결정 보류)**: 댓글·후기 수정 시 편집 표식 미표시. `updated_at` 이 본문 수정 외(상태 변경 등)에도 갱신될 수 있어 오탐 없이 구현하려면 표시 기준·문구를 원장 결정 후 진행.
...
### Changed
- **[문서정합] 세션 마감 — 잔여 문서 drift 정비 + CLAUDE.md 동기화 페어 신설** — R1~R6 변경이 도메인 문서 "현재 상태" 서술에 남긴 잔여 drift 5건 실코드 대조 후 갱신: ① ARCHITECTURE.md sitemap "force-dynamic — cookies 사용"→"ISR revalidate=3600 쿠키리스 anon"(R3-3 실코드 `revalidate=3600`) + RSS ISR/dc:creator 한 줄 병기 ② ARCHITECTURE.md `ScrollManager.tsx` 서술을 `feed-scroll-restore.ts` 연동(풀+scrollTop 스냅샷·소진형 트리거)로 확장(R5-3) ③ TECH_SPEC.md §8.3 유령 라우트 `/settings/notifications`→`/settings/profile`(ProfileEditClient 통합) 정정 + 배지 폴링 `countOnly=1` 서술(R4-4) ④ PRD.md §5.4 "동적 sitemap"→"sitemap(ISR revalidate 3600)", §5.6 접근성에 모달 포커스 트랩·온보딩 실패필드 포커스(R5-2), §4.2 뒤로가기 피드 복원(R5-3) 기재. **CLAUDE.md §5 동기화 페어 신설**: `gen-tag-dictionary.mjs::writeClientSnapshot` 클라 투영 필드 ↔ `procedure-dict.client.ts::SNAP` 타입(R4-3 서버/클라 이원화로 생긴 dual — 클라 소비 필드 추가 시 양쪽 동시 갱신). RSS `dc:creator` 는 직렬화 세부라 PRD 미기재 판단.
...
### Fixed
- **[R5-3] 뒤로가기 시 피드 스크롤·풀 복원** (신설 `lib/feed-scroll-restore.ts`, `ScrollManager` 1줄, `FeedView`) — 앱 셸이 fixed 오버레이 내부 스크롤이라 ScrollManager 의 window.scrollY 저장이 항상 0 + 복귀 시 풀이 초기 20장으로 리셋돼 복원 지점 높이 자체가 없던 문제. 설계: 스냅샷(풀 전체+scrollTop+q/cat, sessionStorage·safe-storage 경유·단일 키·TTL 30분) 저장(pagehide+cleanup 지연저장) → 소진형 트리거 2경로(문서 back_forward + SPA popstate 마크) → paint 전 풀 주입·forPool identity 게이트로 scrollTop 적용. 커서는 orderedIds 선두 prefix 재계산(다른 payload 는 seen-set dedupe 로 self-healing), 좋아요·댓글수는 항상 재조회(스냅샷 불신). PTR·검색·칩 전환·새로고침은 복원 미발화. **Playwright 실측**: SPA POP 60장·y=6300 정확 복원 + 복원 후 무한스크롤 중복 0, 새로고침 최상단, 검색 진입/이탈, 카테고리 피드 복원 전부 통과(문서 back_forward 통합 실측은 dev 한정 하이드레이션 정지 — 운영 대조군 정상 — 로 게이트별 부분 실측 갈음). **검증 중 잠재 결함 2건 추가 발견·수정**: POP 복귀 시 sentinel 관찰자 사망(무한스크롤 정지) · teardown 사이클의 스냅샷 오염. 한계 기록: 문서 back_forward 경로는 하이드레이션 시점 1회 점프 구조적 발생(카드 링크 next/link 전환은 별도 안건). 검수 경고분(HMR 모듈 상태·stable ref 계약 등)은 주석 명문화 반영.
...
### Changed
- **[R4-3] 태그사전 스냅샷(203KB) 클라 번들 제거 — 서버/클라 이원화** (`gen-tag-dictionary.mjs`, 신설 `procedure-dict.client.ts`·`tag-dictionary.client.generated.json` 외 import 재배선 6파일) — 전체 스냅샷이 `Card→category-sets→procedure-dict` 경로로 클라 7곳에 유입되던 것을, prebuild 가 **같은 실행·같은 객체 투영으로 클라 경량 산출물**(category·slug·blacklist·normalizations, drift 구조적 불가)을 함께 생성하고 클라 공용 함수(categoryFor/normalizeTag(s)/isBlacklisted)를 `procedure-dict.client.ts` 로 이동(서버 진입점은 re-export — 서버 호출부 무변경). pubmed·autotag 등 서버 전용 93KB 는 클라에서 완전 제거. **번들 실측: 클라 청크 전체 스냅샷 마커 5→0개, 사전 포함 청크당 −93KB(−39%)**. DB 실패 fallback 에서도 투영 재생성(정합 보장), R1-3 역방향 가드 불변, 4필드 byte 동일 실측. 백로그: 서버 전용 모듈의 클라 import 경계가 주석 의존 — eslint 경계 규칙/번들 분석 CI 도입 검토(검수 권고). 부수 관찰: 스냅샷 `autotag` 필드 소비처(lib/auto-tag.ts)가 코드에 부재(문서에만 잔존) — R6 죽은 코드 라운드에서 확인 예정.
...
### Fixed
- **[R3-3] RSS 표준화 + RSS/sitemap ISR 실효화** (`rss/route.ts`, `sitemap.ts`) — ① RSS `<author>` 에 이메일 아닌 이름(RSS 2.0 위반) → `xmlns:dc` 선언 + `<dc:creator>` 표준 교체, description markdown 원문 노출 → 기존 `stripMarkdown` 재사용, CDATA `]]>` 조기종료 방어(표준 분할 치환 — 검수 권고 반영) ② sitemap: `force-dynamic`+`revalidate=3600` 공존(전자가 후자 무효) + 쿠키 클라이언트가 dynamic 강제 → 기존 `createSupabaseAnonClient`(R-Phase ISR 용) 교체 + force-dynamic 제거 ③ RSS 도 동일 결함 발견(revalidate=1800 무효) → 동일 전환. RLS 동등성 마이그 단위 전수 증명(cards_public_read 0161·get_indexable_tags DEFINER·procedure_reviews 0292 is_public 등 — 크롤러 뷰 기준 동일, 오히려 anon 고정으로 결정적 출력). **빌드 산출물 실측: `/rss` revalidate 1800·`/sitemap.xml` 3600 ISR 전환 확인**(종전 매 요청 dynamic). 부작용: 신규 글 반영 최대 30분/1시간 지연(의도).
...
### Fixed
- **[R2-3] 소형 안전판 묶음** — ① danger 토스트 tone 별 기본 지속시간(1.5→4.5초, 명시 duration 우선 — 의료법 검수 안내 등 장문 가독) ② 댓글 등록/수정/삭제 네트워크 예외 무피드백·unhandled rejection 해소(`CommentsBlock`) — 실패 시 danger 토스트 + 입력·목록 상태 불변(reload 는 재throw 없음 확인 — 오표시 엣지 구조적 차단) ③ 카드 삭제 실패 무언 삼킴 2곳(`Card.tsx`·신스킨 `ui.tsx` PostCard — 지시 1곳 외 동일 패턴 추가 발견) 토스트 피드백 ④ **`lib/safe-storage.ts` 신설**(ss*/ls*, storage 접근 자체 throw·quota 방어) + 무방어 지점 전수 교체(useCardViewer 4·engagement-score 6·impression-queue 2·ReportsDetailView 2 — 인앱 브라우저 크래시 방지, 기가드 지점은 미접촉) ⑤ YouTube fetch 타임아웃(transcript 15s×4곳 fetchWithTimeout·upload-date 10s — abort 는 기존 fallback/warn 경로 자연 합류) ⑥ DoctorProfileView loadMore 10s 타임아웃(FeedView 패리티 — abort 가 R2-1 롤백·재시도 경로로 합류). (마이그 0339) — 정리 경로가 없어 단조 증가하던 테이블(실측 1,848행, 93.6%가 만료분)에 `check_and_increment_rate_limit()` 내 확률 게이트(`random()<0.02`, 약 1/50 호출) 기회적 DELETE 추가. 보존 컷오프 = 48h(최대 window 24h 의 2배 — 활성 카운터 삭제 불가), 기존 `window_start` 인덱스 스캔. 시그니처·SECURITY DEFINER·ACL 불변(적용 전후 실측 대조), 본체(윈도우 계산·upsert)는 0105 원본과 동일 — 레이트리밋 의미 무변화. 1회성 수동 DELETE 는 §10 원칙상 미실행(함수가 트래픽 유입 시 자연 정리 — R2 마감 시 행수 재측정 예정).
- **[R2-2] `/write` 글 유형 탭 전환 시 작성글 무경고 소실 방지** (`skin/write/WriteView.tsx` + WriteTabs/WriteClient/CardEditor/ReviewForm/SkinDiaryForms `onDirtyChange` 배선) — 탭 클릭이 `key={active}` remount 로 폼을 즉시 리셋하던 것 → dirty 면 기존 `UnsavedChangesModal`(edit variant, [계속 작성]/[나가기])로 확인 후에만 전환(보류 중엔 remount 자체가 없어 내용 보존), pristine 이면 종전처럼 즉시 전환. dirty 판정은 CardEditor·ReviewForm 의 기존 `isDirty` 재사용, 항상-무장 정책인 시술노트(DiaryForm)만 탭 전환용 초기값 실질 비교(`hasUserInput`) 신설(폼 자체 이탈 가드는 무수정). optional prop 이라 `/write/[shortcode]`·`/review/*` 등 범위 밖 호출자 무영향. beforeunload/popstate 기존 가드 불변.
...
### Changed
- **[R1-5] 환경변수·문서 stale 일괄 동기화** — ① `.env.local.example`: 코드 실참조 env 8종 추가(YOUTUBE_OAUTH 3종·YOUTUBE_API_KEY·NCBI_API_KEY·PYTHON_* 3종, 참조 파일 경로 주석 포함) + `NEXT_PUBLIC_SITE_URL` 주석 해제 활성화 + 죽은 항목 `NEXT_PUBLIC_NAVER_MAP_CLIENT_ID` 제거(코드 참조 0 — 소비 컴포넌트 자체 삭제됨. Vercel 3개 환경에 값은 잔존 — 무해, 정리는 원장 재량) ② `DEPLOYMENT.md` §10.9(AI 콘텐츠 파이프라인) 신설 — Vercel projects API 실측으로 Production/Preview/Dev 설정 상태 기재, `FIREBASE_SERVICE_ACCOUNT` 누락분 보강(§5 페어 동기화) ③ `ARCHITECTURE.md`: 유령 라우트 2건 정리(`/settings/notifications` — 실제는 `/settings/profile` 내 통합, `/api/dev-sql` — 코드 부재), identity 쿠키 서술을 현행 2쿠키 체계(httpOnly+mirror, sentinel 'primary' 폐지)로, ADR 0014 RENAME "예정"→완료(0186/0187) ④ `PRD.md` §5.2·`DATABASE.md` §3.1: PII 잠금 서술 현행화(7→핵심 8컬럼, anon 0323/0325 + authenticated 0335 + RPC 0334 경유) ⑤ `CommentsBlock.tsx` 주석 1줄 현행화('primary' 잔재) ⑥ 최상위 README Production URL `pbtt.kr`→`pibutenten.kr` (저장소 밖 파일). 코드검수 전수 정합 확인.
...
- **[R1-3/M-2] `tag_normalization` 역방향 8행 정정 + 재발 방지 빌드 가드** (마이그 0338, `scripts/gen-tag-dictionary.mjs`, 스냅샷 재생성) — canonical(=입력 오타여야 함)이 현행 대표어인 레거시 행들이 정상 태그를 쓰기 시점(admin 편집·draft 발행의 normalizeTags)에 다른 태그로 재작성하던 버그. 행별 처분: 뒤집기 2(고압산소→[고압산소치료], 마리오네트라인→[마리오네트주름] — 출력이 비사전이라 반전이 정방향 정규화), 삭제 6(대표어↔대표어 4 + 분해형 2). 지시 7행 외 **가드가 8번째 역방향 행(덴서티알파팁→[덴서티], 신규 등재 직후 자가모순 재생산)을 실작동 적발** — 동일 방침 처분. 가드: prebuild 에서 normalization canonical↔사전 ko 충돌 시 충돌 목록 출력 후 빌드 실패(향후 대표어 승격 시 기존 정규화 행과 충돌하면 배포가 의도적으로 실패해 즉시 드러남. alias-canonical 패턴은 의도적 미탐지 — 주석 기록). production 적용·U+FFFD 오염 0·스냅샷 semantic diff 8키 제거+2키 추가뿐임을 실측. 기존 카드 keywords 소급 교정은 하지 않음(분해 저장된 태그는 사후 식별 불가·현행 표시 무해 — 방지에 집중).
- **[R1-4/M-3] `/api/articles` POST type↔category 모순 조합 차단 — category→type 파생** (`api/articles/route.ts`) — type·category 를 각각 독립 검증만 해 의사·관리자 role 이면 `type='post'+category='qa'` 같은 모순 payload 가 통과 가능했던 방어선 부재(현 데이터 mismatch 0 — 잠복). PUT 과 동일하게 category 를 진실원으로 type 파생(`category==='qa'?'qa':'post'`), 구 type 기준 qa 게이트는 categoriesForRole 403 이 커버하므로 제거. 부수 효과: 모순 payload 가 doctor_slug 필수 검사를 우회하던 구멍 폐쇄. 유일 호출부(WriteClient)는 정합 payload 라 정상 경로 무변화.
...
## [2026-07-04] — 홈 브랜드 소개 띠 (비로그인 전용, SEO 스니펫 개선)

### Added
- **홈 상단 브랜드 소개 띠** (`components/skin/HomeBrandIntro.tsx` + FeedView `topSlot` prop) — 홈에 "눈에 보이는" 브랜드 소개 텍스트가 없어(sr-only h1 만) 구글이 시술 카드 본문을 "피부텐텐" 검색 스니펫으로 발췌하던 문제 해소. 상단(로고/검색 밑, 피드 위)에 옅은 브랜드 톤(--primary-soft) 소개 띠 1개. **비로그인/크롤러에게만 노출**(useSession() null 게이트), **고정 아님**(스크롤 시 사라짐) → 로그인 회원 앱 피드 무영향. 비검색 홈(`/`, `!query`)에서만. 문구는 앱스토어 등록 포지셔닝 일치(store-listing.md §2: 피부날씨→피부다이어리→전문의 Q&A→시술후기): "피부 날씨부터 시술 후기까지, 하나의 피부 커뮤니티 / 오늘의 피부 날씨 · 나의 피부 다이어리 · 피부과 전문의 Q&A · 시술 후기를 한 곳에서."
...
### Changed (2차 — 같은 날)
- 요약 줄 "효과" 항목 줄바꿈 교정(원장 확정): 한글 음절 단위 개행이 "효/과"처럼 글자 중간을 자르던 것 → 효과 라벨+값 묶음(whitespace-nowrap)으로 통째 개행 — 자리가 모자라면 다음 줄이 "효과 …"로 시작
- **댓글 메타 위치 이동(원장 확정, 2단 조정)**: 시간·답글·♡가 우측 float 로 첫 줄에 끼어 부자연스럽던 것 → **본문 마지막 문장 끝에 인라인으로 이어 붙게**(별도 새 줄 1차 시안은 "녹아들지 않는다" 피드백으로 재조정 — 자리 없으면 묶음째 자연 wrap). ⋮ 메뉴만 우상단 유지. CommentItem 공통이라 피드 미리보기·펼침·답글·글상세 전 화면 적용. 실측: 본문 끝 6px 옆 같은 줄 배치 확인
- 요약 줄 색 톤 조정(원장 확정): 재시술·회복(다운타임) 문구를 하늘색 → 연한 회색(--text-muted)으로 — 값 전부 파란색이면 산만. 별점·통증 값·효과 값은 하늘색 유지. 실측: 회색 rgb(162,166,175)/통증 하늘색 유지 확인
...
- **표 정돈(원장 실사용 피드백)**: 전 칸 **중앙정렬 통일**(좌/우 혼재 제거) · **만족도 분포 바 삭제**(의미 전달 약함 — 5→1점 분포 수치는 만족도 숫자 호버 툴팁으로만 유지) · **카테고리 그룹 헤더 제거**(항상 플랫 목록) · **기본 정렬 = 후기수 내림차순**(헤더 클릭 2단계 토글로 단순화 — 해제 단계·기본 순서 버튼 제거)

디렉터 프로세스: 마이그+화면 병렬 구현 → 디비전문가(적용 가)·코드검수관(치명 0, 권장 5건 반영) → 0328 적용·실측 검증 → tsc·build 통과.

### Added
- **/admin/review-reports 컬럼 확장** — 생성일(YY.MM.DD) · **만족도 평균+5→1점 미니 분포바**(같은 칸, 툴팁 상세) · 다운타임(최빈 라벨 — 동률 시 짧은 쪽) · 효과 top3(가운뎃점 연결+툴팁 n). 폭은 3칸만 추가(min 합계 ≈752px — 1280px 데스크탑 무스크롤)
- **헤더 클릭 정렬** — 숫자 8칸(후기수·재시술%·만족도·통증·조회·저장·공유·생성일) 내림→오름→해제 3단계, 정렬 중 카테고리 그룹을 풀어 플랫 목록 + "기본 순서" 복귀 버튼, null 값 항상 하단. 61행 규모라 클라 정렬
- 마이그 0328: get_review_report_overview 4필드 확장 — DATABASE.md 표 참조. 화면은 마이그 미적용 상태에서도 안전 폴백('—')

### Changed
- **대시보드 상단 "시술 리포트" 숫자 카드 → /admin/review-reports 직행**(원장: 전용 표 진입 동선 — 종전엔 카드 목록으로 가서 못 찾는 문제). 하단 운영 프로그램 📊 카드와 진입점 2개
- 검수 반영: 행 key en 기준(플랫 정렬 중복 방어) · HeadCell aria-label(정렬 방향) · 타입-정규화 일치(view/save/share number) · 주석 정정 2건
...
로컬 E2E 실측(첫 20장 Q&A 정확히 6장 · "댓글 7개 모두 보기" 렌더+클릭 펼침 · 콘솔 에러 0) → tsc·build 통과.

### Added
- **전체 피드(/) Q&A 슬롯 보장** — `lib/feed-shuffle.ts::blendQaQuota`(결정적·랜덤 없음): 매 20장 창마다 전문의 Q&A 최소 6장을 균등 페이싱으로 보장. organic 이 자연 공급한 Q&A 도 창 할당량에 산입(자연 점수로 충분하면 강제 삽입 안 함), id dedup, 출력 길이 = 원 풀 유지. page.tsx 전체 풀 분기가 전체+Q&A 풀 Promise.all → blend → 기존 diversifyByDoctor 순 적용(캐시 키 v3). 카테고리 탭(/?cat=)은 미적용. 배경: 시술후기 750건 유입 후 최근성 점수로 Q&A 가 풀에서 소멸(4/300) — 가중치 튜닝만으론 데이터 나이에 따라 재발해 슬롯 보장으로 해소(인스타식 피드 섞기)
- **댓글 "댓글 N개 모두 보기" 줄 복원**(인스타 표준, 원장 요청) — 피드 카드 미리보기(인기순 3개) 하단에 숨은 댓글이 있을 때만 절제된 텍스트 줄 표시, 클릭 시 전체+입력창 펼침(💬 토글과 진입점 2개). `CommentsBlock` optional `onExpandRequest`(미제공 구형 호출자 무영향) + PostCard 위임. 검수 반영: stale 응답 가드·조건 명시화

### Changed
- **의사글 가중치 x2 → x3** (마이그 0327, production 적용·실측 검증 — DATABASE.md 표 참조). 자연 top300 내 Q&A 4→6장(보조 효과 — 주역은 위 슬롯 보장). 인기태그(사이드바)·토픽 keywords 집계가 의사글 키워드 쪽으로 소폭 이동 가능(의도된 결과, 디비전문가 판정)
- blendQaQuota 검수 반영: 전체 풀 RPC 실패 시 즉시 빈 배열(종전 패턴 — 실패 풀로 blend 진행 금지), Q&A 풀만 실패 시 organic 전용 degrade(피드 생존)

### 개발 참고
- 로컬 E2E 중 **PWA 서비스워커(pbtt-static-v1/dynamic-v2)가 dev 에서도 등록되어 낡은 클라이언트 청크를 서빙**하는 문제 발견(서버 재시작·캐시 헤더와 무관하게 지속, SW 해제 후 정상). 위생 백로그: SW 등록을 production 전용으로 게이트


## [2026-07-03] — 운영 제보 3건 정정: 피드 카테고리 서버 풀(/?cat=) · 댓글수 신선도 · 투데이 KPI 링크

원장 실기기 제보(Q&A/끄적끄적 탭 씨마름 · 댓글수 0 고착 · 투데이 타일 탭 불가) 원인 확정 후 일괄 정정.
디렉터 프로세스: 병렬 구현 2패키지 → 디비전문가(마이그 사전) + 코드검수관 2렌즈 + SEO검수관 3중 교차 검수 →
[치명] 1건·채택 경고 4건 반영 → 재검수 승인 → 로컬 브라우저 E2E(Q&A 300풀·무한스크롤 20→80·끄적끄적 18/18·댓글배지 1·콘솔 에러 0) 검증.

### Fixed
- **피드 카테고리 탭 씨마름(구조 결함)** — 종전 "상위 300 풀 1개를 탭이 클라 필터" 모델이 2026-06 시술후기 750건 유입으로 풀이 review 290/300 도배되며 Q&A 4개·끄적끄적 6개만 표시(무한스크롤도 더 줄 것 없음). **카테고리 탭 = `/?cat=` URL 라우팅(URL 이 SSOT) + 서버 카테고리별 300 풀(마이그 0326 `p_category`)** 로 전환. FeedView 의 구 기계장치(검색해제 chipExplicitRef·①-b 자동로드·sentinel 칩별 분기) 삭제, URL→chip 단방향 싱크에 A2 sessionStorage 복원(첫 실행 1회) 통합. 검수 반영: 빈 상태/스켈레톤 조건의 구 모델 잔재 교체(빈 카테고리·빈 검색 스켈레톤 무한표시 여지 제거), poolEpochRef(풀 교체 중 stale loadMore 응답 폐기 — 옛 카테고리 카드 혼입·stale hasMore/에러 오염 차단), 전환 중 sentinel 게이트(catPending — 곧 버려질 옛 풀 낭비 fetch 방지)
- **댓글 수 낡음 고착** — 카드 댓글배지가 배치 미리보기(1회 수신 후 ref 로 영구 스킵) 값에 갇혀 PTR(당겨서 새로고침)·서버 새 풀 도착 후에도 재조회되지 않던 버그(제보: 댓글 달렸는데 0, 펼쳐야 갱신). 풀 리셋 시 previewFetchedRef/viewerFetchedRef 초기화 — 새로고침·검색·카테고리 전환마다 댓글 미리보기·좋아요/저장 상태 재조회
- **투데이 KPI 타일 4개 링크화** — 내 노트/내 후기/내 글/내 댓글 타일이 표시 전용 div 라 탭해도 무반응(미구현). 마이페이지와 동일 규칙(SSOT)으로 연결: /notes · /{handle}?tab=reviews|posts|comments (핸들 미설정 회원은 프로필행 3개 비링크 폴백, 게스트 분기 무변). aria-label + 탭 피드백
- **FeedSidebar 인기 Q&A 빈 섹션** — 카테고리 풀(시술후기·끄적끄적)엔 의사 Q&A 가 없어 헤더만 남던 것 → 빈 배열이면 섹션 미렌더

### Added
- `lib/feed-categories.ts` — 피드 카테고리 슬러그·라벨·검증 SSOT(검수 반영: page.tsx·FeedView 이중 정의 drift 제거). 마이그 0326 은 DATABASE.md 표 참조
- `/?cat=` 메타: noindex,follow + canonical=홈(검색 ?q= 와 동일 밸브 원칙) + '카테고리' 피드 title. clinics JSON-LD 는 홈(/)에서만 렌더(SEO 검수 반영 — 엔티티 신호 집중)

### 운영 참고
- GA4 는 q 만 제거하고 cat 은 보존 — 홈 트래픽 집계 시 `/?cat=` 페이지뷰가 분산 집계됨(카테고리 분석 가능해진 대신 단순 홈 합산 시 포함 필요)
- 배포 순서: 마이그 0326 production 적용·실측 검증(전체 300 하위호환·qa 300·doodle 18·ACL 명시·U+FFFD 0) 후 코드 push


## [2026-07-03] — 가입 닉네임 기본값 비움 + 비로그인 모달 CTA 정리 (소급 기재)

### Changed
- **이메일 가입자 닉네임 기본값 비움**(`signup/page.tsx`, b5a48d9) — 이메일 로컬파트가 공개 표시명으로 노출되는 것 방지, 본인이 직접 작성(원장 결정). provider 판별은 app_metadata + user_metadata(Naver 자체 흐름) 이중 참조 — 소셜 가입자 이름 폴백은 유지
- **비로그인 모달 /signup 직링크 제거**(fd61147) — LoginPromptDialog 단일 CTA("3초 만에 시작하기 — 가입 / 로그인" → /login 통합 시작 화면), EngagementPromptDialog 는 소셜 버튼 1차 + 이메일 가입/로그인 텍스트 링크 보조. 비로그인 클릭 시 /login 으로 튕기던 /signup 직링크 잔재 코드베이스 전체 0 확인


## [2026-07-03] — 온보딩 피부톤 "얼굴 골라 선택" UI (원장 확정)

### Changed
- **피부톤(Fitzpatrick) 질문 UI 전면 교체** (`OnboardingClient.tsx`) — 질문을 "내 피부색과 가장 가까운 얼굴을 골라주세요."로, 긴 반응 설명문 6줄(2줄×6종·인종 표기) 리스트 → **코드 SVG 얼굴 아이콘 6개(3×2 그리드) + 짧은 캡션**("북유럽·켈트계 매우 흰 피부"~"아프리카계 짙은 갈색 피부"). 이미지 에셋 0(로딩 무부담). 톤·캡션은 신규 SSOT `lib/fitzpatrick.ts`. 저장값(1~6)·전파 RPC 무변.
- 필수 정책은 전부 기존 유지(얼굴형·피부타입·피부톤·피부고민·관심시술·이메일 필수 / 사진·한줄소개 선택) — 피부톤 에러 문구만 "피부색을 선택해주세요."로. 검수 [치명](이메일 검증 중복 삽입) 즉시 제거 — 기존 검증(빈값+형식)이 이미 필수 보장.
- **후속 확정(650e548·0a9df34, 원장 실기기 피드백)**: 색상 v2 로 조정(#FDF0E6/#F9E6D6/#F1D6B8/#D2AC82/#8B5A33/#422818) + 톤 이름 제거(캡션 1줄만). **선택 표시 복구** — AppShell `.root` unlayered 버튼 리셋이 Tailwind 유틸을 죽여 클릭해도 선택이 안 보이던 것을 인라인 스타일(2중 테두리+하늘 배경+✓ 배지)로 우회. 검수 3건 반영(hover 복원 `hover:!border-*` · ✓ 배지 AA 대비 `--primary-active` · transition 정리).

### Removed
- **편집기 `.tmp.*` 잔재 58개 전체 삭제**(src/·supabase/ — gitignore 된 로컬 파일, grep·IDE 검색 오염원. 감사 P8 위생 항목 선행 처리).
...
### Added
- **시술 리포트 허브 `/reports` 신설** — 내비 '리포트' 슬롯 진입점(ReportsHubView). 시술 리포트 목록을 후기 N≥4(FEED_MIN_REVIEWS) 게이트 통과분만 count desc(동률 ko 사전순)로 표시. `force-dynamic`(라이브 집계), 자격 0건이면 noindex(빈 허브 색인 회피). N=4~9 는 '데이터 쌓이는 중' 미세 표기. 개별 리포트는 동적 라우트 `/reports/{ko}` 와 공존. ※비주얼 리디자인·헤드라인 풀 엔진은 옆 세션(reports-new 임시 라우트)에서 진행 중 → 추후 `/reports` 이식 예정(현재 미이식).
- **시술 직후 반응(reactions) 필드 (마이그 0320)** — `procedure_reviews.reactions text[] DEFAULT '{}'` 컬럼 신설, `create_procedure_review`/`update_procedure_review` RPC 에 `p_reactions` 파라미터 추가(오버로드 충돌 방지 위해 DROP 후 재생성 + GRANT 재부여). 후기 폼에 '시술 직후 반응' 다중선택(부기/멍/딱지/붉어짐·홍조/화끈거림·열감/멍울·뭉침 + '없음' 단독선택) 도입.
- **공유 자동완성 키보드 네비 훅 (`useAutocompleteKeyboard`)** — 자동완성 목록 ↑↓ 하이라이트 이동 + Enter 선택 + 한글 IME(isComposing/keyCode 229) 가드. 헤더 검색·시술후기 시술선택·시술노트 병원검색이 공유(SSOT, SkinDiaryForms 병원검색 패턴 추출).

### Changed
- **내비 5탭: '마이'→'리포트' 교체** — 하단 탭바·데스크탑 GNB 모두 투데이·내 노트·피드·리포트·쇼핑 5탭. '마이'는 탭에서 빠지고 헤더 우상단 아바타로만 진입. 로고는 데스크탑=`/`(피드)·모바일=`/today` CSS 토글. 쇼핑 탭=준비중(딤드 + 클릭 시 안내 토스트, 텍스트 배지 없음). (AppShell.tsx)
- **검색: 별도 `/search` 페이지 폐기 → 인-헤더 통합** — 검색을 AppShell 헤더 인-헤더 입력으로 통합(검색 실행은 홈 `/?q=` 라우팅). 전역 규칙 통일: ←=검색 닫기(나가기), ✕=검색어만 지움. 입력↔결과 알약 픽셀 동일. 검색 결과=피드+시술 리포트 블렌딩(시술명 매칭 시 결과 최상단에 리포트 1장).
- **검색 발견패널(SearchPanel) 리스킨** — 배경=피드 그라데이션 캔버스로 통일. 카테고리=박스 없는 가로 텍스트 탭(선택만 브랜드색 2.5px 밑줄, no-scrollbar). 키워드 칩=활성 카테고리 옅은 틴트(흰색 칩 폐기). 최근검색 칩 시계 아이콘 제거. 인기검색어 표시 제거.
- **피드: review_summary 홈피드 직접 주입 제거** — review_summary 카드는 홈 피드에 직접 주입하지 않고 검색 블렌딩에서만 노출. FeedSidebar 인기태그=시술 6종 한 줄.
- **시술후기 폼(ReviewForm) 정리** — 다운타임 질문 필수→선택(시술 직후 반응에 증상이 1개 이상일 때만 조건부 노출, 미선택=NULL 저장·집계 무영향). 시술 선택=검색 입력+자동완성+카테고리별 인기 칩(상위 18) 방식, 시술명이 폼 맨 위. 문구 교체: 상단 "소중한 후기로 다른 분을 도와요", 반응 질문 "시술 직후 불편한 점은 없었어요? (선택)", 제출 버튼 "올리기". 독려 멘트 복원, 단답 글자수 카운터 제거, "잘 기억 안 나요"→"기억 안나요". 임시저장 자동복원 제거(항상 빈 폼으로 진입).
- **글쓰기 FAB(WriteFab) 노출 확장 + 전환 갭 제거** — 화이트리스트에 `/today`·`/reports`(+하위) 추가(투데이·내 노트·피드·리포트 4개 노출). 숨김 브레이크포인트 `sm:hidden`(640px↑)→`min-[900px]:hidden` 으로 데스크탑 헤더 '글쓰기' 버튼(≥900px)과 일치 → 640~899px 전환 갭(글쓰기 진입 사라짐) 제거.
- **시술후기 폼 추가 폴리시** — 제출 버튼을 앱 표준 하늘색 버튼(시술노트와 동일 `h-11`·항상 `--primary`·`disabled:opacity-60`)으로 통일(비활성 회색 분기 제거). 다운타임은 반응 토글에 따라 grid-rows 로 살며시 열림/닫힘. '언제쯤' 칩 컴팩트화('기억 안나요' 단독 줄바꿈 방지)+두 줄 회색 통일+2단(연중) 페이드 등장. 단답 질문 교체 시 질문·예시문 동시 등장.

### Fixed
- **[주의] 수정 모드에서 반응 전체 해제 시 다운타임이 DB 에 잔존하던 클라↔RPC 규약 충돌 정정 (마이그 0321)** — `update_procedure_review` 의 downtime 갱신 CASE 가 `COALESCE(array_length(p_reactions,1),0)=0` 이라 빈 배열 `{}`(반응 전체 해제=명시적 비움)일 때도 기존 downtime 을 보존 → 신규 클라이언트가 보낸 reactions=[]+downtime=null('둘 다 비움' 의도)이 'reactions=빈배열+downtime=잔존' 불일치로 남았다. 조건을 `p_reactions IS NULL`(구 클라이언트=미전달일 때만 보존, 빈 배열은 p_downtime 반영)로 좁혀 정정. 시그니처 동일이라 CREATE OR REPLACE(본문 나머지·GRANT 동일). UTF-8 경로 적용·U+FFFD 0·오버로드 1개 확인.
- **카테고리 탭 첫 항목 좌측 잘림** — `justify-center`→`justify-start sm:justify-center` 로 모바일 첫 항목 좌측 클리핑 해소.
- **헤더 검색 시 종·아바타 2px 밀림** — `.headerInner` 모바일 gap 8 로 통일.
- **투데이 하단 글상자 잘림** — `.page` 하단 safe-area 보정.
- **시술노트 병원 입력 키보드 가림** — onFocus `scrollIntoView` 로 포커스 항목을 키보드 위로 노출.
- **입력 포커스 '이중 박스' 제거** — `.root input/textarea:focus-visible { outline:none }` 일반화로 검색 알약 등 텍스트 입력의 3px 안쪽 링 제거(래퍼 `focus-within` 보더로 포커스 표현). 버튼/링크 a11y 포커스 링은 유지.
- **데스크탑 검색 드롭다운 배경 통일** — `.searchSuggest` 배경 `#fff`→`var(--tt-canvas) fixed`(모바일 패널과 동일). 흰 드롭다운 위에서 안 보이던 흰 최근검색 칩 가시성 해소·모바일/데스크탑 검색 외관 일관.
- **헤더 검색 자동완성 키보드 네비** — SearchPanel 이 forwardRef+useImperativeHandle 로 키다운 핸들러 노출, AppShell 데스크탑/모바일 검색 입력이 위임(하이라이트 Enter=선택 시 기존 runSearch 미실행). 자동완성 활성 하이라이트를 안 보이던 색→`var(--primary-soft)` 로 가시화.
...
### Removed
- **추천의향(recommend)** — 모든 폼(독립 후기/시술노트/체크인)에서 UI 제거. 재시술의향(revisit)으로 통일. DB 컬럼·API 스키마는 하위호환 유지.
- **효과 시기(effectOnset)** — 모든 폼에서 UI 제거. "효과는 언제부터 느끼셨어요?" 질문 삭제(첫날 알 수 없음, 만족도와 중복). 리포트 집계(EffectOnsetTimeline)는 기존 데이터로 유지.
- **효과 체감도(effectFelt)** — 시술노트·체크인에서 UI 제거. 만족도(satisfaction)와 중복.
- **"평가만 익명으로 공개할게요" 토글** — 시술노트 ReviewAccordion에서 제거. 기본 공개(isPublic: true)로 변경.

### Changed
- **폼 제목**: "시술 후기를 남겨주세요" → "내 시술, 기록으로 남기기"
- **제출 버튼**: "후기 올리기" → "내 기록 남기기" / "후기 수정" → "기록 수정"
- **효과 라벨**: "이번 시술로 달라진 점을 모두 골라주세요!" + 서브텍스트 → "달라진 점을 골라주세요" (간결화)
- **시술노트 후기 버튼**: "이 시술 후기 쓰기 (안 쓰면 기록만 남아요)" → "이 시술, 기록을 남겨볼까요?"
- **한줄후기 placeholder 3개 교체**: 동기 부여형 문구로 변경 ("별로였다면 별로였다고...", "아팠으면 아팠다고...", "광고는 안 알려주는 걸...")
- **임시저장 복원**: 무조건 자동 복원 → confirm 대화상자("이전에 쓰다 만 기록이 있어요. 이어서 쓸까요?")로 변경. 거부 시 draft 삭제.
- **API 안전성**: `p_effect_onset` 전달 시 `?? null` 폴백 추가 (reviews/route.ts, reviews/[shortcode]/route.ts). visits 경로와 패턴 통일.

---

## [2026-06-28] — 태그 카테고리 6종 확대 실행 계획서 작성

### Added
- **`docs/PLAN-tag-category-overhaul.md`** — 시술 카테고리 2종→6종 전면 개편 실행 계획서 (809줄). 독립 검수관 4명 교차 검수 완료. DB 마이그레이션 4개(0311-0314) + 코드 29파일 + 문서 5개 변경 설계. 사용자 태그 JSON 수령 후 다음 세션에서 실행 예정.
...
### Added
- **팔로우/구독 (원장·회원 상호)**: 명함(profile.id) 단위로 서로 팔로우. 마이그 0290 + 0291 — `follows` 테이블 + RLS(**RPC-only**: 직접 접근 deny, SECURITY DEFINER RPC만. 0291 검수 정정으로 GRANT 없던 죽은 공개SELECT 정책 제거) + `toggle_follow`/`get_my_follow` RPC(`toggle_card_save` 동형: 묶음검증·active fallback·자기팔로우 차단) + `follow_post` 알림 kind(`notification-kinds.ts` 동기화) + **발행 트리거**(`on_card_publish_for_followers`: 팔로우 대상이 글 발행 시 follower 들에게 알림 개별 INSERT, 자기자신 skip, INSERT/UPDATE 양 경로). 마이그는 트랜잭션 ROLLBACK 으로 트리거(INSERT·UPDATE 전환·재저장 무중복) 실증 후 적용.
  ...
  - **새 글 알림 끄기 토글**(마이그 0292, SNS 표준): 알림 설정에 "팔로우한 분 새 글" on/off. `notification_preferences.pref_follow_post` + `is_notification_enabled` 게이트(끈 사람은 발행 트리거에서 제외) + get/save prefs RPC 10번째 컬럼·인자(기존 9-인자 유지=무중단 배포). 알림함에 "새 글" 필터 칩 추가.
...
- **홈 피드 풀·리포트 풀 캐시 (#2, SNS 표준 분 단위 갱신)**: 비검색 홈이 매 방문마다 임계경로에서 돌리던 `feed_cards_scored`(300행 점수계산) + `get_review_summary_pool`(46행 집계)를 쿠키리스 anon + `unstable_cache`(revalidate 90s)로 분리(위 인기태그 패턴 확장). 두 무거운 공개 쿼리를 캐시에서 즉시 반환 → 홈 서버 임계경로 단축. **per-user 좋아요/저장(viewerStates)은 캐시 밖으로 분리**(아래 클라 배치 이전) — 캐시 본문엔 개인 데이터 미포함(캐시 오염 없음). 피드 신선도는 90초 윈도(디렉터 승인: 엄청 신선 불필요). 전체 정적 ISR 은 홈이 `searchParams.q`(검색)를 읽어 본질적으로 dynamic 이라 비대상 — 데이터 캐시가 실질 이득.
  ...
  - **viewerStates 완전 클라 배치 이전 (SNS 표준)**: 홈 SSR 이 임계경로에서 돌리던 viewer 좋아요/저장 prefetch(`fetchViewerStatesRecord` + `auth.getUser()`)를 제거 → 신규 `GET /api/viewer-states?cardIds=`(viewer-states.ts SSOT 재사용·sparse·비로그인 0쿼리)로 **FeedView 가 마운트 후 1회 배치 조회**(댓글 미리보기 배치 패턴 미러, N+1 없음 — 홈 카드 `useCardActions` 는 카드별 self-fetch 자체가 없음). 결과: 비검색 홈 SSR 이 **완전 사용자 무관·경량**(auth·쿠키 파싱 없음 → 검색 분기로 격리). 카드는 배치 도착 시 `useCardActions` 의 동기화 effect(interactedRef 로 사용자 토글 보존)로 좋아요/저장 반영. SSR=클라 초기=false 라 하이드레이션 불일치 없음. 독립 검수 2명 차단 0(권고로 page 서버클라 검색분기 이동·route SSOT 재사용·card_likes RLS 주석 정정 동반).
...
- **온보딩 저장 버튼 비활성 버그**: 프로필 사진 미등록 시 저장 버튼이 disabled 상태로 고정되어 직원들이 저장 불가하던 문제 수정. 프로필 사진을 필수에서 선택으로 변경 (OAuth 프로필 이미지가 기본, 없으면 기본 아바타 표시). 저장 버튼은 항상 클릭 가능하며, 미충족 항목 클릭 시 구체적 안내 메시지 표시
- **비로그인 좋아요/저장 안내**: 토스트 → LoginPromptDialog 모달로 변경 (Phase 5 B1)
- **빈 검색결과 CTA 하드코딩 색상**: CSS 변수로 전환 (Phase 5 B2)
- **[Item 7a] 시술후기 "다시 선택" 전체 초기화 버그**: reselectProcedure()가 만족도·통증·다운타임 등 모든 필드를 초기화하던 문제 수정. 이제 선택한 시술만 초기화하고 나머지 입력값은 보존
- **[Item 7b] 시술후기 효과 칩 간격**: 칩 간 gap-2(8px) → gap-3(12px)으로 확대하여 가독성 개선
- **[Item 8] 시술노트 입력창 이중 테두리**: focus:ring-0 추가하여 Tailwind 기본 포커스 링과 border가 겹쳐 이중선으로 보이던 현상 해소 (inputCls, inputSm, textareaCls 3곳)
...
- **입력 폰트 통일**: inputCls(14→16px) + inputSm(13→16px) + textareaCls(14→16px) 전체 16px 통일 (iOS Safari 자동 줌 방지)
...
- **댓글 영역 빈 공간·구분선 깜빡임(피드백 9-나·9-라)** — (1) 좋아요줄↔댓글 사이가 `.comments` 래퍼(16/16+선)와 CommentsBlock 자체 `mt-3 pt-2.5` 가 겹쳐 ~54px 빈 공간. (2) `cards` 에 댓글 수 컬럼이 없어 미리보기 fetch 전엔 0 → 선을 먼저 그렸다 지우는 깜빡임. → 선·여백을 `.comments` 가 아니라 **`.commentsActive`(commentCount>0 또는 입력 열림 시에만)** 에 부여 + 안쪽 이중여백 제거. 0댓글 카드는 선이 처음부터 안 뜨고, 댓글 카드는 댓글과 함께 등장. 프리뷰(모바일) 실측 검증(0댓글 border 0 / 댓글카드 border 1px). (commit `b5a2371`·`9c583e6`)
...
>
> **근본 원인**: 같은 날 02:28 배포한 `/api/weather` 서버 프록시(`77b4b59`). 프록시는 모든 사용자 요청을 **Vercel 서버의 공유 egress IP** 한 곳으로 모은다. Open-Meteo 무료 티어는 "IP당 일일 한도"로 차단하는데, 그 IP는 전 세계 수많은 Vercel 앱이 공유하므로 **우리 트래픽이 적어도 다른 테넌트와 합산**으로 한도가 소진돼 502. 캐시된 기본값(대치동 — 전 사용자 공유)만 항상 성공하고 사용자별 고유 좌표(캐시 미스)는 실패 → "전원 대치동". 무료/per-IP-제한 서드파티 API를 공유 서버리스 IP로 프록시한 것이 안티패턴.

### Fixed
- **`fetchWeather`**(`src/components/skin/record/skin-weather/weather-logic.ts`): `/api/weather` 단일 호출 → **Open-Meteo 2곳 클라이언트 직접 호출로 환원**(사고 전 검증된 동작). 각 사용자가 자기 휴대폰/집 IP(분산)로 호출 → 개인이 한도에 닿지 않음. 콜드로드 속도는 `useWeather` 의 localStorage seed + stale-while-revalidate 가 보완(재방문 즉시).

### Removed
- **`src/app/api/weather/route.ts` 삭제** — 환원으로 사용처 없음. (`77b4b59` 의 프록시 라우트 폐기.)

### 예방 원칙 (ADR [0021](decisions/0021-no-free-api-proxy-through-shared-serverless-ip.md))
- **무료/per-IP-제한 서드파티 API는 공유 서버리스(Vercel) IP로 프록시하지 않는다.** 캐시·속도가 필요하면 (a) 클라이언트 직접 호출 유지 + 클라 캐시, (b) API 키 기반 유료 플랜(키당 한도), (c) 자체 데이터 캐시 레이어 중 택일. 공유 IP 합산 한도는 우리 트래픽과 무관하게 터진다.

---

## [2026-06-24] — 검색창 하나로 통일 (알림·아바타 항상 유지, 검색은 그 앞에서만 열림)

> 검색 입력 모드가 헤더 전체를 **풀폭 입력**으로 교체해 알림·아바타가 사라지고 입력창이 우측 끝까지 늘어났다. 입력 모드와 결과 모드의 검색창 레이아웃이 제각각이라 "검색창이 상황마다 다르다(누더기)"는 문제. 한 검색창을 여러 상황에서 재사용하도록 정리(원장 결정).

### Changed
- **헤더 우측 액션 한 벌 추출**(`src/components/skin/AppShell.tsx` `headerActions`): 알림 벨 + 아바타/로그인 JSX 를 `headerActions` 한 벌로 추출해 **검색 모드·기본 모드가 공유**(중복 JSX 제거).
- **검색 입력 모드에 알림·아바타 유지**: 검색 입력 모드에도 `headerActions` 를 두어 알림·아바타가 그대로 노출되고, 검색 입력(`.headerSearchInline` `flex:1`)은 **그 앞 공간만** 차지(풀폭·우측끝 제거). → 입력 모드(`[검색입력][알림][아바타]`)와 결과 모드(`[검색어 알약][알림][아바타]`)의 헤더 레이아웃이 **동일**해짐. 데스크탑(상시 검색 pill)은 변경 없음.

---

## [2026-06-24] — 검색창 통일 + 입력 글씨 16px (iOS 확대 방지)

> 검색 입력 모드와 결과 모드의 검색창 모양이 달랐고(입력 모드에만 ← 뒤로 버튼), iOS 가 16px 미만 입력창 포커스 시 화면을 자동 확대해 글씨 크기가 들쭉날쭉했다.

### Changed
- **검색창 모양 통일**(`src/components/skin/AppShell.tsx`): 검색 입력 모드의 **← 뒤로 버튼 제거**, ✕ 를 항상 노출 + 누르면 **검색 닫기**(입력이 비어도 닫힘). 입력/결과 두 상태의 검색창을 동일하게(원장 결정).
- **검색 글씨 16px**(`src/components/skin/app.module.css` `.headerSearchInline input`·`.mobileQueryText`): 14px → **16px**. iOS 는 16px 미만 입력창에 포커스하면 화면을 자동 확대 → placeholder·타이핑 글씨가 커 보였다가 결과 화면에서 작아 보이던 문제. 16px 로 확대 차단 + 입력↔결과 글씨 크기 일치.
...
### Added
- **`src/components/NativeStatusBar.tsx`**(layout 마운트): 네이티브에서 웹뷰 로드 후 런타임으로 `StatusBar.setStyle({ style: Style.Light })`(=밝은 배경용 어두운/검정 아이콘) 호출. 빌드 시점 config 보다 안정적으로 적용(특히 Android 15 edge-to-edge). 배경(overlay·헤더색) 미변경 — 글씨색만. 웹/PWA 는 no-op(동적 import 가드).

---

## [2026-06-24] — 헤더+칩바 한 요소 병합 (스크롤 모션 '한 덩어리'화)

> 모바일에서 스크롤 시 헤더(상단 1열)와 칩바(2열)가 따로 움직여 '두 층이 접히는' 어색함이 있었다. 같은 transform 을 줘도(개별 동기 이동) 둘이 **분리된 sticky 요소**라 칩바의 sticky 흐름 전환 중 미세하게 어긋났다(원장 리포트). 두 바를 **하나의 래퍼 요소로 병합**해 통째로 움직이게 하여 어긋남을 구조적으로 제거.

### Changed
- **헤더+칩바를 `.topStack` 래퍼로 병합**(`src/components/skin/AppShell.tsx` + `app.module.css`): 칩바를 `.page` 에서 빼내 헤더와 함께 `.topStack`(sticky top:0) **한 요소** 안에 배치. 스크롤 숨김 시 래퍼 전체를 `transform: translateY(-62px)`(=헤더 높이) 한 번에 이동 → 헤더는 상태바 필러(`.root::before`) 뒤로 가려지고 칩바는 맨 위(`--sat`)에 정위치. **한 요소라 헤더·칩바가 픽셀단위로 함께 움직여 어긋남·틈이 구조적으로 불가능.** `.header`/`.chipBar` 는 정적(sticky·transition 은 래퍼로 이전), 칩바는 풀폭 + 데스크탑 max-width 1080 중앙 정렬. 검색 모드(`searchOpen`)에선 칩바 미표시. `--sat=47` 시뮬레이션으로 평상시 flush(gap 0)·숨김 시 래퍼 통째 -62 이동·헤더 가려짐·칩바 `--sat` 정위치를 모바일·데스크탑·검색 모드에서 확인.
  - 개발 중 거쳐간 단계(칩바 `top`→`transform`, `.chipBar::before` 틈가림, 헤더·칩바 개별 lockstep `-56`)는 모두 이 병합으로 대체·정리.
...
### Added
- **iOS App Store 정식 출시 (대한민국)**: Apple 심사 통과("1.0 배포 준비됨 / Ready for Distribution") 후, App Store Connect 가격 및 사용 가능 여부에서 **대한민국 1개국 가용성 ON** 저장 완료. 가용성 페이지 "사용 가능 여부(1개의 국가 또는 지역)", 대한민국 상태 **"처리 중에서 사용 가능"**(최대 24시간 내 스토어 반영). 빌드 7(iPhone 전용 `1.0 (7)`), App ID `6781289580`, Bundle `kr.pibutenten.app`. 가용성 설정 작업은 "Claude in Chrome" MCP 통제 탭에서 진행, 최종 게시 "확인" 클릭은 원장님이 직접 수행.
- **Android Google Play 정식 출시 (대한민국)**: 프로덕션 트랙(버전 1 / 1.0) 6/19 제출분이 6/21 Google 검토 통과·자동 게시 완료. Play app ID `4975379335091010610`, applicationId `kr.pibutenten.app`.

---

## [2026-06-20] — 네이티브 앱 상태바 띠 비침 4차 수정(헤더 숨김 스크롤 상태)

> 3차 수정(아래 블록, `--sat` 주입)으로 상태바 겹침은 해소됐으나, 원장 리포트로 **새 증상** 확인: 오류는 아니고 모바일에서 아래로 스크롤해 헤더가 사라진 상태일 때, 화면 맨 위 상태바 띠(0~`--sat`) 뒤로 스크롤되는 피드 내용이 비쳐 보였다(시계·날씨 영역에 콘텐츠 고스팅). **정적 코드 분석으로 원인 확정**: 헤더 숨김 시 `.header` 가 `translateY(-100%)` 로 화면 밖으로 슬라이드되고 칩바는 `.chipBarUp(top:var(--sat))` 로 띠 바로 아래 고정 → 0~`--sat` 띠에 불투명 배경이 사라져 그 뒤 `.root` 가 깔아둔 fixed 캔버스 그라데이션 위로 스크롤되는 카드가 그대로 비침. 헤더가 보일 때는 헤더 베이스색(불투명 #e8f5fd + 동색 반투명 틴트 = 사실상 솔리드)이 띠를 덮으므로 헤더 숨김 상태에서만 발생. z-index 전수 감사로 그 띠를 정당하게 점유하는 다른 요소가 없음을 확인.

### Fixed
- **상태바 띠 비침 4차 수정 — 헤더 숨김 스크롤 상태**(`src/components/skin/app.module.css`): `.root::before` 불투명 필러(`position:fixed; top:0; height:var(--sat); background:var(--tt-canvas-top); z-index:60; pointer-events:none`)를 추가해, 헤더 유무와 무관하게 0~`--sat` 띠를 항상 헤더 베이스색으로 덮는다. z-index 60 은 헤더(40)·칩바(30)·검색제안(50)·모바일검색패널(45) 위, 모달(1000)·토스트(9999) 아래로 배치(탭바 60 은 화면 하단이라 공간 충돌 없음). `.root` 엔 transform/filter 가 없어 `position:fixed` 가 뷰포트 기준 고정. **웹/PWA 회귀 없음** — `--sat`=0 → 필러 높이 0px(preview eval 로 확인).

### Changed
- **헤더 베이스색 토큰화**(`src/components/skin/app.module.css`): 세 곳(`.root` 캔버스 그라데이션 시작점 · `.header` background-color · `.root::before` 필러 배경)에 하드코딩돼 있던 `#e8f5fd` 를 신규 토큰 `--tt-canvas-top` 단일 출처로 묶음. 향후 헤더/상태바 띠 색 변경 시 한 곳만 고치면 세 곳이 항상 일치(이음매 색 어긋남 방지, 코드검수 [권장] 반영). 해석값 동일이라 시각적 변화 없음.
...
### Fixed
- **Android 네이티브 앱 상태바 겹침 해결**(`capacitor.config.ts`, `android/app/src/main/assets/capacitor.config.json`, `android/app/src/main/java/kr/pibutenten/app/MainActivity.java`): Android 15(API 35)+ 부터 edge-to-edge 가 **강제**(앱 targetSdk 36)되어 WebView 가 상태바·내비게이션 바 뒤까지 그려지고, 웹 화면 최상단이 OS 상태바(시계·배터리)와 겹쳤다(PWA 는 무관 — 원래 정상). **근본 원인**: 1차 시도로 `MainActivity` 에 추가했던 커스텀 `ViewCompat.setOnApplyWindowInsetsListener` 가 Capacitor 8 코어 `SystemBars` 플러그인의 inset 리스너를 **덮어쓰고**(뷰당 리스너 1개) 정작 자신은 `requestApplyInsets()` 미호출로 발동되지 않아 실기기에서 무효화됨. **수정**: ① `MainActivity` 의 커스텀 inset 코드를 전부 제거해 bare `extends BridgeActivity {}` 로 원복 — Android 15+ 에서 Capacitor 코어 SystemBars 가 WebView 부모(CoordinatorLayout)에 systemBars inset 패딩을 올바른 시점(`onPageCommitVisible` + `requestApplyInsets()`)에 적용한다(겹침 해소의 실제 메커니즘). ② StatusBar 설정의 SSOT 는 `capacitor.config.ts`(추적 파일) — 번들 산출물 `android/.../assets/capacitor.config.json` 은 `android/.gitignore` 로 추적 제외이며 CI 의 `npx cap sync android` 가 `.ts` 에서 매 빌드 재생성한다(로컬 일관성 위해 동일하게 맞춰둠). `overlaysWebView:false`·`backgroundColor:#ffffff` 는 Android 15+ 에서는 edge-to-edge 강제로 무시되고 Android 14 이하·iOS 에서만 유효(겹침·배경색 보조). 신규 의존성 없음.
- **상태바 아이콘 색 가시성 수정**(`capacitor.config.ts`, `android/app/src/main/assets/capacitor.config.json`): StatusBar `style` 을 `DARK` → **`LIGHT`** 로 정정. Capacitor 의 `Style.Dark`(=`"DARK"`)는 "어두운 배경용 **밝은(흰) 텍스트**" 의미라 흰 배경(#ffffff)에서 시계·배터리 아이콘이 보이지 않았다. `Style.Light`(=`"LIGHT"`)= "밝은 배경용 **어두운(검은) 텍스트**" 로 바꿔 흰 상태바에 검은 아이콘이 보이도록 함. `setStyle` 은 Android 15+ 에서도 적용된다.
...
- **원장 프로필 "답변 N편" 헤더 글씨 축소·경량화**(`src/components/skin/app.module.css`): 데스크톱·모바일 양쪽 헤더(`.doctorAnswerHeader`, `.doctorAnswerHeaderMobile`)를 `18px/800` → **`15px/600`** 으로 더 얇고 작게(사용자 요청 "전체적으로 좀더 얇고 작았으면"). 데스크톱은 공용 `.backTitle > *`(18px/800)을 소스 순서 우선으로 덮음(`.backTitle > *` 자체는 다른 페이지 공유라 미수정). 숫자(`<b>`)는 Tailwind preflight(`b,strong{font-weight:bolder}`)로 부모 600 기준 `bolder→900` 이 되어 여전히 투박하던 것을 `.doctorAnswerHeader b, .doctorAnswerHeaderMobile b { font-weight: 700 }` 으로 명시 덮어 본문(600)보다 살짝만 강조(900→700).
...
### Changed
- **색 스케일 통일**(`weather-logic.ts`): `uvRamp` 시작색 청록(#27B4A6)→초록(#2FA968). 신규 `sevStep(sev,steps)`(양자화)·`sevUvb/sevUva/sevPm25`(심각도 0~1)·`colorTop`(4단계)·`colorWeek`(8단계). UVB 태닝·UVA 노화·미세먼지 3지표를 상단 4단계/주간 8단계 동일 램프로 통일. 구름투과율은 정보값이라 파랑(#2E86C8) 유지.
- **라벨**: "UVB 홍반"→"UVB 태닝" (chips·kpis·OVERLAYS·주간 박스·KPI_INFO 라벨). 설명 prose의 '홍반'(의학 용어)은 유지.
- **강수확률 제거**: today 요약 카드 하단(`BG_KEYS`에서 precip 제거)·주간 카드 💧 제거. chips/kpis 의 precip 항목·`WeatherKpi.key` 의 `"precip"` 타입 제거.
...
> 모바일에서 "검색 결과 상태"와 "검색창 열림 상태"가 다른 창처럼 보이고, 검색바에 커서를 대도 동작이 일관되지 않던 문제를 근본 정리. 결과 상태=검색어 표시+결과 피드(발견 메뉴 없음), 검색바 탭=기존 검색어 비우고 발견 메뉴(최근/인기/카테고리) 노출, ✕=피드 복귀. 데스크탑은 무변경. `tsc` 0·`build` 0·코드검수 [치명] 0.

### Changed
- **AppShell 모바일 검색 상태 분리**(`components/skin/AppShell.tsx`): 확정 검색어(`value`=URL `q`, 결과 피드·검색어 알약 구동)와 입력 초안(`draft`, 신규)을 분리. 모바일 검색창 열기(`openMobileSearch`: 검색 아이콘·결과 알약 탭)는 `draft=""`로 시작해 발견 메뉴를 띄움 → 결과 피드(URL `q`)는 흔들리지 않음. 모바일 열림 input/패널은 `draft` 사용, 열림 내부 ✕는 초안만 비움(완전 종료는 ← 뒤로). 데스크탑 상시 pill·드롭다운은 기존 `value` 그대로(무변경).
...
- **/today(RecordView 재구성)**: 날씨 요약 → 인사 히어로 → 관심 키워드 새 글 → 인기글. 히어로 = 인사 한 줄(연한 글씨) + 우상단 '내 노트 보기 ›'(/notes, 연한 글씨) + 히어로 내 KPI 4종 박스(라벨 위·숫자 아래 = 날씨 칩 일관, 반투명). 히어로 액션 버튼은 제거(글쓰기는 FAB/사이드바로 분리). KPI 순서 **내 노트·내 후기·내 글·내 댓글**. 기존 KPI "내 글에 달린 댓글"(받은 댓글, myCardIds 추적) → **"내가 쓴 댓글"**(comments.author_id=active·status=visible)로 교체. 데스크탑 사이드바 = 글쓰기 3박스(노트 작성하기/후기 작성하기/글 올리기), 모바일에선 웰컴 카드 바로 아래로(min-[900px]:hidden).
...
- **원장 공개 프로필(`/doctors/[slug]`)**: 모바일에서 프로필(소개) 카드를 답변 피드 **위**로(데스크탑 2단 레이아웃은 현행 유지). 카드 본체 1회 정의·재사용 + 900px 분기 CSS 클래스 2개 추가(모바일=본문 위 H2 / 데스크탑=사이드바 H1, 페이지 H1 1개 유지).

### Fixed
- **피드 인기 태그 클릭 검색 회귀**: 사이드바 '인기 태그'를 누르면 그 태그로 검색(`/?q=`)되며 재마운트로 서브 카테고리 탭이 '전체'로 풀리던 문제 — `FeedSidebar` 의 `tagTab` 을 `sessionStorage`(`pbtt:feedSidebar:tagTab`)에 보존해 검색 후에도 선택 탭 유지. 헤더 검색·BetaDiscovery 경로 무변경.
...
### Added
- **오늘의 피부 날씨** (`/record/weather` 상세 페이지 + 내 노트 상단 카드). Open-Meteo 2종 API(air-quality + forecast, 키 불필요·CC BY 4.0)만 사용, 런타임 LLM 없음. 피부 관점 4지표(UVB 홍반·UVA 노화·미세먼지·구름투과율)를 "절대값" 대신 "최대 범위 대비 현재 위치 게이지"로 표현(UVB/UVA 는 오늘 최고치도 연한 채움+마커). 시간별 겹쳐보기 그래프(과거 24h~향후 24h, 스크롤·드래그·휠·클릭 readout), 주간 피부 날씨(날짜 클릭 선택). 핵심 로직 `weather-logic.ts`(joinHourly·uvaFromSW·BLOCK_LUT·MSG bank·tempColor 영하 -15까지 확장), 측위·캐시 훅 `useWeather`(geolocation + BigDataCloud 역지오코딩으로 실제 동단위 위치, sessionStorage 30분 TTL, 카드↔상세 스냅샷 공유), 컴포넌트 `SkinWeatherCard`/`WeatherDetail`/`WeatherDetailView`.
- **시술노트 자세히 페이지** (`/record/notes`). 내 노트 hero "내 노트 보기" → 별도 페이지에서 3토글(전체/시술노트/끄적끄적) 타임라인 전체보기(`RecordNotesView`/`RecordNotesPanel`).
- **게스트 인기글 노출**: 비로그인 상태에서도 내 노트에 인기글 섹션 표시(흥미 유발) + 그 아래 가입 CTA. `buildPopularData` 의 published 필터 + RLS 로 anon 안전(비공개 row 자동 제외).

### Changed
- **베타 스킨 운영 승격(커토버)**: 홈(`/`)·내노트(`/record`)·글쓰기(`/write`)·원장 대시보드(`/doctor`)·공개 SEO 3종(의사프로필·토픽·리포트)·신뢰/법적 안내 10p·의사목록·알림·후기작성·글상세 3라우트·진입(로그인·가입·온보딩)·프로필 클러스터(`/[handle]`·`/my`·`/settings`)를 신규 베타 스킨으로 전환. 기존 스킨은 `/old-skin` 으로 백업 보존.
- **토픽·리포트·원장 프로필 단일열 통일**: `/topics/[tag]`·`/reports/[procedure]` 를 2열 Masonry → 홈 피드와 동일한 단일열 `PostCard` 리스트 + 우측 홈 피드 사이드바(인기태그·인기 Q&A·오늘의 피부 한줄)로 통일. 브레드크럼 제거, 제목을 `< 뒤로` 옆 `backTitle` 슬롯에 배치(마침표 없음). 원장 프로필(`/doctors/[slug]`)은 글상세와 동일 구조 — 좌측 답변 단일열 피드(무한스크롤), 우측 원장 카드 사이드바(사진·이름·프로필 더보기·함께 보면 좋은 Q&A). 공용 사이드바 `FeedSidebar` 추출.
- **글쓰기 3탭 상단 레이아웃 통일**: 시술노트·시술후기·끄적끄적의 제목 위치·폼 시작 위치·폼 너비(→680px)·타이틀 상승 효과를 끄적끄적 기준으로 통일. 시술노트 부제 제거.
- **뒤로 버튼 '<' 통일**: `BackButton` 에 `hideLabel` prop 추가('뒤로' 텍스트 숨기고 '<' 화살표만), `BetaSkinShell` 에 `backTitle` 슬롯 추가(뒤로 옆 페이지 제목). 여백·색상 정합.
- **작성자 클릭 같은 창 이동**: PostCard 작성자 링크 `target="_blank"` 제거 → SPA 같은 탭 이동(유독 원장만 새 창 열리던 문제 해소).
- **인기태그 서버 고정 · 더보기 베타 톤 스켈레톤**: 인기태그를 서버에서 결정해 새로고침 흔들림 제거, 로딩 스켈레톤을 베타 톤으로 교체.

### Fixed
- **더보기 슬라이드 롤백** (`0216201`): polish 에서 도입한 max-height 슬라이드가 더보기 펼침을 깨뜨리고(클릭해도 안 펼쳐짐) 접힘 가장자리를 하드컷으로 만든 회귀 → 원래 line-clamp 방식으로 복원.
- **인기태그 클릭 시 목록 변경 버그** (`a1da194`): `feed_cards_scored` 의 jitter 0.35(랜덤 신선도)가 요청마다 인기태그 풀을 다르게 계산 → 태그 클릭 시 사이드바 전체가 바뀜. 인기태그 풀에 한해 jitter 0(결정적) 적용(피드 카드는 jitter 유지).
- **topKeywords 클라이언트 경계 500** (`62dc3b6`): `topKeywords` 가 "use client" 모듈(`FeedSidebar`)에 있어 서버 페이지(topics/reports)에서 호출 시 SSR throw(빌드는 통과·런타임 500). 비클라이언트 모듈 `feed-sidebar-data.ts` 로 분리해 해소.
- **날씨 현재값 정합·온도색**: 대시보드 현재값을 그래프의 "지금"과 동일 인덱스에서 파생(불일치 제거), UVB/UVA 오늘 최고 게이지 추가, 온도색 기준을 영하(-15)까지 확장.
- **날씨 상세 마무리(본 세션 미커밋분)**: 위치·기온·최저/최고/습도 헤더를 하나의 글상자로 묶고 정보를 좌우로 펼침(좌측 치우침 해소), 오늘의 피부 팁을 같은 글상자 안 구분선 아래 미니멀하게 통합(라벨 제거), 4지표 사이드바 설명을 괄호 없는 자연어로 재작성, "구름투과" → "구름투과율" 명칭 통일.
...
### Fixed
- **사이드 인기 태그 위치 고정**: 태그 클릭=검색이라 pool 이 검색결과로 바뀌면 빈도 재계산되어 클릭한 태그가 맨 위로 올라오던 문제. `BetaSkinFeed` 의 `popularTags` 를 `hasFrozenRef` 플래그로 "비검색 상태 최초 1회만" 고정 → 태그를 눌러도 순서·구성 불변(무한스크롤 증가분 미반영은 의도된 trade-off).
- **검색 시 피드 스크롤 최상단 복귀**: 스크롤 컨테이너가 window 가 아니라 셸 `.root`(overflow:auto)라 `window.scrollTo` 가 무효였던 문제. 모듈 헬퍼 `scrollFeedTop`(스크롤 가능한 첫 조상 탐색) 추가해 칩(탭) 전환·검색(searchQuery) 변경 시 `.root` 를 top:0 으로.
- **"함께 보면 좋은 Q&A" 연관성**: feed 24장 풀에서 뽑아 연관도 0(무관)까지 노출되던 문제(힐로웨이브 글에 쥬브젠 Q&A 등). `post/page.tsx` 를 전용 쿼리로 교체 — 같은 영상(`video_id` 직접 일치, byId select 에 video_id 추가) 우선 → 키워드 겹침(`.overlaps`) 순 상위 3개. 연관 0 채우기 없음(없으면 섹션 숨김).

### Notes
- 검증: 빌드·tsc 통과, 코드검수 치명 2건(frozen 조건·임베드 필터) 반영 후 dev 실측 — 인기태그 클릭 후 순서 불변, 검색 시 스크롤 560→0, 힐로웨이브(2246) 연관이 같은 영상 3개(2244·2245·2247)로 정확. 헤더 hide-on-scroll 은 원래 정상(모바일 숨김/복귀·데스크탑 고정)임을 Playwright 실측 확인.
...
- **내 일기(/record) 비로그인 게스트 열람(가입 유도)**: 기존 `redirect("/login")` 폐지. 게스트는 '예시' 데모 타임라인(하드코딩 가짜 데이터, 클릭 시 `/signup`) + 가입 히어로/하단 CTA + 인기 키워드(`getPopularByCategory`) 기반 공개 최신 Q&A 예시. 인기글 섹션(authenticated RPC)·대시보드는 게스트 숨김. noindex 유지.
...
### Added
- **시술일기 저장 동작**: `/api/diaries` POST(zod 검증 `lib/schema/api/diaries.ts` + rate limit + tag_dictionary 매칭 → `create_diary` RPC). DiaryForm 실제 저장 배선 + 저장 후 `/record` 이동 토스트. 시술일기 폼 키보드 포커스 자동이동(병원→원장→실장→시술명, IME 조합 제외).
- **내 일기(/record) 실데이터화**: 서버에서 active 명함 diaries+procedures 조회 → 연표/달력/목록 패널이 실제 데이터 렌더(목업 더미 제거). 빈 상태 + 달력 월별 파생·이동. 비로그인 → /login.
- **시술일기 상세 `/record/[id]`**: 본인 소유 1건 조회(RLS), 날짜·병원·의료진·전화/지도·받은 시술별 가격·메모·일기 본문. 목록/연표/달력 항목 클릭 → 상세 이동(`openDetail`).
- **내 일기 홈 인사+상태문구 카드(Figma)**: 마지막 시술명+방문일로 5단계(다운타임/효과발현/유지기/기록없음/장기미수) 자동 판정(시술 5종+fallback). 리마인더·관심사 Q&A 캐러셀은 데이터 공백분 더미(자리만). 연표/달력/목록 유지.
- **알림(/notifications) 2탭**: 상단 '내 기록 / 활동' 탭. '활동'=기존 알림 시스템(필터·무한스크롤·읽음처리·선택모드) 그대로 보존, '내 기록'=주기·예정·답변 알림 자리(현재 더미).
- **마이페이지 '내 활동' 4탭**: 작성글·댓글·좋아요·저장. `ProfileTabs` 재사용 + `activityOnly` 옵션(내 후기·내 피부 제외). 서버 prefetch(작성글+4종 카운트, 본인 기준).

### Changed
- **마이페이지 '프로필·계정 설정' 인라인 폼**: 별도 `/settings/profile` 이동 없이 아코디언으로 펼쳐 그 자리서 수정(`ProfileEditClient` `embedded` 임베드, 'h1 내 정보' 헤더 숨김). 기존 '내 활동' 단순 링크는 '바로가기'(알림·내 프로필)로 축소.
...
### Added
- **검색 + 카테고리 동시 필터** (마이그 0277): `/beta` 검색 결과 화면에서 카테고리 탭(전체/Q&A/시술후기/끄적끄적/리포트)을 눌러도 검색어가 유지되며 해당 카테고리로 좁혀짐. `search_cards_scored` RPC 에 `p_category` 인자 추가(NULL/'' 이면 기존 전체). 첫 페이지(`/beta/page.tsx`)와 무한스크롤(`/api/cards`→`Feed`)이 동일 필터 집합 사용(페이지네이션 일관성). 리포트 탭은 시술별 통계라 시술명(한글/영문) 부분일치로 리포트 풀 필터, 결과 없으면 "‘…’ 검색 결과가 없습니다" 표시. `fetchCardList`·`/api/cards`·`Feed` 에 `category` 인자/prop 추가.

### Changed
- **모바일 검색 헤더 깜빡임 제거**: 검색어 선택/제출 시 `router.push` 직전에 `urlQ` 를 낙관적으로 세팅 — 오버레이 닫힘과 URL 반영 사이에 헤더가 잠깐 홈(로고)으로 돌아가던 깜빡임 제거. 검색바는 그대로 유지되고 아래 목록만 결과로 교체.
- **모바일 검색 입력 스타일 통일**: 돋보기→열린 검색 입력을 결과 화면의 회색 알약 바와 동일 스타일로 통일(열림/결과 간 모양 변화 제거).
- **상단 네비 로고 위 여백**: `pt-1.5` 소량 추가.
- **탭 전환 체감속도 개선**: `/beta` 에 loading 경계가 없어 탭 클릭 시 무반응처럼 보이던 문제 → 칩 `<Link>` 에 `useLinkStatus` 기반 즉시 스피너(클릭한 칩 단위 — Next 소스 확인, navigation-global 아님). `page.tsx` 독립 쿼리 병렬화(전체 탭 `feed_cards_scored`+리포트풀 `Promise.all`, `getHotQaIds`/`headers` 선행 발화 후 일괄 수확). 발견 칩 크기 축소.
- **검색창 열림 매끄럽게**: `/api/beta-discover` 를 열 때마다 재fetch 하던 깜빡임 제거 — 모듈 캐시 + `/beta` 진입 시 `prefetchDiscover()` 선로딩 → 재열기 즉시 표시.
- **자동완성 안내문 제거**: 입력 시 매칭이 없으면 "‘…’로 시작하는 검색어가 없어요" 대신 발견 화면(카테고리 등)으로 폴백.
- **밑줄 폭 조정**: 칩줄 활성 밑줄 `left/right-[6px]`→`0`(칩 전체 폭), 발견 카테고리 탭 밑줄 좌우 -4px 확장("댕강" 제거).
