...
### Changed
- **리포트 정렬 칩을 피드처럼 헤더 칩바 슬롯으로 이전** (3d625aa, 원장 제보 — "리포트 최상단도 피드처럼 두 층이 붙어 함께 움직이게"): 허브 정렬 칩을 본문 sticky → **셸 헤더 칩바 슬롯**(AppShell chips=.topStack)으로 이전해 헤더와 한 요소로 이동/숨김(모바일 동시 숨김·데스크탑 상시 고정, 피드와 동일 경험). 정렬 상태를 ReportsShell 로 리프트(ReportsSortContext, 기존 category 패턴)·본문(ReportsIndexView)은 구독만·뒤로가기 스냅샷 복원 유지. 칩은 피드처럼 버튼 배열 그대로 전달(group 래퍼 없이 aria-pressed). 상세 후기 정렬 칩은 헤더에 못 올리므로(최상단=히어로) '**후기 헤더+정렬칩'을 한 sticky 블록으로 묶어** 후기 구간 동안 함께 고정(부유 칩 어색함 해소). `.canvasReport .chipBar` 배경 #F5FBFF 동색. 문서: PRD "칩바=헤더 한 덩어리" 표준으로 정정.
