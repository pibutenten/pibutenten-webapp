...
- `ProcedureReportCard`(서버 컴포넌트) — 순서: **재시술 의향(상단, 만족도보다 살짝만 강조)** → 만족도(별점+분포) → 통증(부드러운 파스텔 바) → 많이 본 효과(제목 크게·% 차분) → **작성자 통계(남녀·연령대 비율)** → 면책. 헤더는 시술명(좌)+후기 수(우) 한 줄, 뱃지 '피부텐텐 리포트'. 전체 시각 무게 균형 조정.
- `ProcedureReviewStream`(클라이언트): 리포트 아래 개별 후기를 **댓글 아님 + 풀 카드 아님 — 컴팩트 목록**(작성자·날짜 / 요약 한 줄 / 본문, 좋아요·댓글·공유 없음). Q&A 카드처럼 접힘(앞 3개 + "후기 N개 더보기"). 항목 클릭 시 원문 이동. 데이터는 후기 한 벌만.
