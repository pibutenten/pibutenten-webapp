...
- **미지정 태그 거버넌스 — '검토'(triage) 탭(발주 E·F, 마이그 0269)**: `reviewed_at`(미검토/검토완료). 태그 관리 상태 칩 '검토'(status=triage) 전용 화면 = 미검토 미지정. 추천 토글 + '검토완료(잔류)'/'되돌리기'(관리 칸 가로) + 미검토 개수·포함보기. 미지정(unspec) 화면은 일반 조회로 원복. 분류·병합·삭제는 기존 행 편집. (대량 일괄 분류 기준은 여전히 별도 안건.)
