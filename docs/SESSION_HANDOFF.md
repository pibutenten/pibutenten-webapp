...
- **미지정 태그 거버넌스 — 트리아지 흐름 구축(발주 E, 마이그 0269)**: `reviewed_at`(미검토/검토완료). 태그 관리 미지정 필터에서 추천 토글 + '검토 완료(잔류)'/'되돌리기' + 미검토 기본 목록·포함보기. 분류·병합·삭제는 기존 행 편집. 운영자가 미검토분을 점진 소진하는 방식. (대량 일괄 분류 기준은 여전히 별도 안건.)
