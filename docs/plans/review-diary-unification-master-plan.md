...
| D-H | **v1: diary_linked 후기는 비공개(`is_public=false`) 추이그래프 전용**, 공개 후기는 standalone만 허용. 롤업이 공개 결론칸을 month4까지 사후 변동시켜 `/reports` ISR·JSON-LD `aggregateRating`이 stale로 남는 캐시·SEO 정합 경로가 미완이므로, 공개 시계열 후기는 P3 이후로 미루고 그때 `upsert_review_checkin`의 `revalidatePath` 계약(§3.3)을 선결한다(기술 검증 [치명]+major 반영). **★F3로 반전됨(2026-06-27 야간 원장 최종결정) — diary_linked 공개 허용. `create_visit_with_entries`(마이그 0297)는 `is_public=true`면 cards(type=review,category=review)+review_summary 앵커 lazy 생성 후 card_id 세팅(`diary_linked_public_not_allowed_v1` 게이트 제거). 단 Phase 3 API의 `upsert_review_checkin` 호출부(`/api/reviews/checkins`)가 `revalidatePath('/reports/{en}'+family)` 필수 — 롤업 사후변동으로 인한 ISR·JSON-LD `aggregateRating` stale 해소는 본 RPC가 아니라 API 레이어 책임.** |
...
| diary_linked 공개 후기 | 암묵 허용(공개 시계열) | **★F3로 허용(D-H 반전, 2026-06-27 야간)** — `is_public=true`면 카드/앵커 lazy 생성(마이그 0297). 롤업 사후변동 캐시·SEO 정합은 `upsert_review_checkin` 호출부의 `revalidatePath`(Phase 3 API 책임)로 해소. (구안: v1 차단·공개는 standalone만 — 폐기) |
...
6. **시술별 후기 루프** (`p_reviews`):
   ...
   - **★F3로 반전됨(2026-06-27 야간): diary_linked 공개 허용**. 아래 두 분기는 모두 마이그 0297에 구현되어 있으며, `is_public` 값에 따라 분기합니다. `diary_linked_public_not_allowed_v1` 게이트는 제거되었습니다.
   - **비공개 분기 (`is_public=false`)**: 통합 작성 후기를 `card_id=NULL`로 `procedure_reviews` INSERT(`visit_id=v_visit_id`, `source='diary_linked'`, `date_precision`, `is_public=false`, `diary_procedure_id`, `solo_price`, 결론 칸 부분입력 가능). **카드·앵커 미생성**(추이그래프 전용).
   - **공개 분기 (`is_public=true`, F3)**: 라우트가 마스킹·검수·shortcode 생성 후 `p_reviews[*].card`(title/body/keywords/status/shortcode/post_year)를 주입하면, RPC가 `cards(type=review,category=review, shortcode, post_year)` INSERT로 `card_id` 확보(create_procedure_review 본문 재사용) 후 동일 INSERT 절(`is_public=true`, `card_id`, `source='diary_linked'`, `visit_id=v_visit_id`)에 앵커 lazy 생성(`status='published'`일 때 `ON CONFLICT (post_slug) WHERE type='review_summary'`)을 추가합니다. day0 checkin·트랙A 예약은 공개/비공개 공통으로 동작합니다. **전제: F3의 revalidate 계약(§3.3) — `upsert_review_checkin` 호출부 `/api/reviews/checkins`가 `revalidatePath('/reports/{en}'+family)` 수행(API 레이어 책임).** 공개 standalone 회고 후기는 여전히 §3.6의 `/api/reviews`→`create_procedure_review` 경로로 별도 작성.
...
### 5.4 "달라진 점" 분석 — 현재 범위 제외 (추후)

> **원장 지시(2026-06-27): "달라진 점" 연령별·효과별 베스트시술 분석 대시보드·분석 RPC(`get_change_analysis`)는 지금 만들지 않는다. 현재 구현 범위에서 제외.** 데이터가 충분히 쌓인 뒤 별도 단계에서 착수한다.

**단, 데이터 수집은 코어로 유지한다** — `procedure_reviews.effect_areas`(결론칸, 666건 100% 채움) + `review_checkin.changed_points`(시계열)는 계속 모인다. 따라서 추후 분석에 필요한 원천 데이터는 v1부터 정상 축적된다. 분석/대시보드 착수 시 적용할 원칙만 남긴다(설계 상세는 본 커밋 이전 git 이력 참조):
- 집계는 결론 칸(`procedure_reviews.effect_areas`, `is_public=true AND card_id IS NOT NULL`)만 읽고 `review_checkin` 미참조.
- §5.5 표본 게이트(≥4) 통과 후 공개, **병원별 분해 금지**(`diaries` JOIN 안 함), 가격 미참조(F2), EFFECT_NONE_LABEL 제외.
