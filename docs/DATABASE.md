...
| `clinic_id` | bigint → clinics(id) | `role='clinic'` 병원 명함의 소속 지점(건보 심평원 코드 참조·불변, `doctor_id` 와 대칭). ON DELETE SET NULL. anon 차단·authenticated SELECT 부여(비-PII, 0348). 마이그 0342 |
| `legal_name` | text | ★선택 복원(0342, 구 0110 drop 을 clinic 배치에서 재도입). 회원이 온보딩에 입력하면 유지(쇼핑몰 주문·결제 대비), 미입력 허용. CHECK 1~50자. **PII** — anon/authenticated SELECT 차단, 본인·관리자는 RPC 경유. 탈퇴 시 NULL 스크럽(0347). 병원 매칭은 이 값에 비의존 |
...
- ~~`legal_name` — 0110 drop (이메일 dedup 전환)~~ → **0342 에서 재도입**(선택 복원용 실명, PII·CHECK 1~50자 — 위 스키마 표 참조). 현재 존재 컬럼.
...
### 1.6. 알림 / 푸시
- `notifications`, `notification_preferences` (0062, 0063, 0079, 0080). kind 는 text+CHECK — 0244 8종(comment/reply/like/save/review_request/published/report/**keyword**) → 이후 `follow_post`·`diary_reminder`(0300) → **0346 clinic 2종 추가로 현 12값**: 위 + `clinic_link_request`(병원 연결 요청)·`clinic_visit_added`(병원 새 시술노트 대행 작성). `notification_preferences` pref 컬럼: comment/reply/like/save/review_request/published + 관심 알림 3종 **keyword_interest/keyword_concern/keyword_skin_type**(0244, default ON) + `pref_review_checkin`·`pref_diary_incomplete`(0300) + **`pref_clinic_link_request`·`pref_clinic_visit_added`(0346, default true)**. (※ report 는 전용 pref 없이 상시 수신. 병원명 비노출은 알림 표시 코드에서 처리.)
...
- `doctors`, `doctor_accounts`, `videos`
  - **`doctors` 병원 3토글**(마이그 0341): `clinic_id`(bigint FK→clinics ON DELETE SET NULL — 근무 지점 건보 심평원 코드 참조·불변, 시술노트 드롭다운 대상)·`is_affiliated`(bool DEFAULT true — 재직/소속, 퇴사 시 false → 드롭다운 제외)·`is_listed`(bool DEFAULT true — 공개 페이지 on/off, 퇴사와 독립). 로직 판정은 `clinic_id` 로만, 기존 `clinic`/`branch` text 는 표시·레거시 보존. `is_listed=false` = 공개 표면 전체 404(공개 필터). `doctors_clinic_id_idx` 부분 인덱스. authenticated 는 SELECT, service_role 은 `clinic_id/is_affiliated/is_listed/branch/slug` UPDATE(0348).
- `youtube_oauth_tokens` (0097 — DB 이전)
- `content_reports` (0137 — 신고 큐)
- `audit_logs` (0140 — 민감 API 1년 보관)
- **`clinic_member_links`** (0344 — 병원 계정 · 시술노트 대행): 병원(지점 계정)↔회원 명함 연결(비귀속 다대다) + 병원 보유 환자 기록. 컬럼: `id` bigserial PK / `clinic_id` FK→clinics CASCADE / `profile_id` FK→profiles CASCADE(회원 명함) / `status`(pending/active/rejected/revoked) / `created_by_clinic_profile_id` FK→profiles(등록 병원 계정) / `consent_at`·`consent_version` / 병원 입력 대조·감사용 `requested_legal_name`·`requested_birthdate` / `registration_number`(병원 내부 챠트번호·고유값 아님·탈퇴 존치) / 병원 직접입력 `patient_phone`·`patient_address` / 회원→병원 스냅샷 `patient_name`·`patient_birthdate`·`patient_email`·`patient_skin_profile`(jsonb) / `created_at`·`revoked_at`. 인덱스 2(clinic_id·profile_id) + 부분 UNIQUE(`clinic_id, profile_id`) WHERE status IN(pending,active). **RLS ENABLE + anon/authenticated REVOKE ALL(정책 0개 = 직접 접근 완전 차단)** — 접근은 전부 SECURITY DEFINER RPC 0345(후속) 경유. 회원 유래 PII 스냅샷은 탈퇴 시 NULL 스크럽(0347).
...
- **`diaries`** (시술일기/방문, 0278 신설 + 0292·0302·**0343** 확장): 기존 컬럼(`id`/`profile_id` FK profiles/`clinic_id` FK clinics nullable + 병원 텍스트 스냅샷/`doctor_name`/`manager_name`/`diary_body` ≤400/`created_at`/`updated_at`) + **0292 추가 7컬럼** `clinic_home`/`clinic_kakao`/`total_price`(int ≥0)/`is_complete`(bool DEFAULT true — 미완성 임시저장 트랙 B)/`reminder_stage`(smallint)/`reminder_muted`(bool)/`visited_on_precision`(text CHECK) + **0343 병원 대행 5컬럼**(전부 additive): `source`(text NOT NULL DEFAULT 'member' CHECK member/clinic — 작성 주체, 병원 대행분=clinic)/`created_by_clinic_profile_id`(uuid FK profiles ON DELETE SET NULL — 대행 입력한 병원 계정 감사)/`linked_consent_at`(연결 동의 시각 스냅샷)/`next_appointment_date`(date — 재방문 리마인더)/`doctor_id`(uuid FK doctors ON DELETE SET NULL — 병원 모드 담당 원장, source='clinic' 전용). ⚠ `diaries.source`(member/clinic=작성 주체) ≠ `procedure_reviews.source`(standalone/diary_linked=후기 파생) **동명·다른 도메인**. **`visited_on` 은 0292 시점 NOT NULL → 0302 에서 DROP NOT NULL**(precision='unknown' = 날짜 미기억 회고 후기는 visited_on NULL 허용). `visited_on_precision` CHECK 는 0292 의 `exact`/`season`/`half`/`year` → **0302 에서 `unknown` 추가**(5종). 비공개 owner-only RLS(active 명함 단위). 일기 삭제는 raw DELETE 가 아니라 `delete_visit` RPC 전용(0292 에서 `diaries_delete_own` RLS DELETE 정책 제거 = FIX-1, 0297 `delete_visit` 으로 일원화). ⚠ 병원의 회원 소유 diaries 생성(source='clinic')은 RLS 우회 SECURITY DEFINER RPC 0345(후속) 전용 — RLS 자체는 0343 에서 미변경.
...
> ⚠ **번호 중복 사실 (재번호 금지)**: **0015 / 0046 / 0047 / 0048 / 0049 / 0292 / 0299** 는 서로 다른 마이그 **각 2파일**이 같은 번호를 공유한다 (0004b·0044 는 한 마이그의 의도적 파트 분할이라 별개). 모두 production 적용이 끝난 이력이므로 **파일 rename(재번호) 절대 금지** — 적용 이력 파괴. 이후 신규 마이그 번호는 **기존 최대 번호 +1** 을 엄수한다 (`scripts/check-migration-naming.mjs` 가 pre-commit 에서 신규 충돌만 차단, 기존 위반은 `scripts/migration-naming-baseline.json` 허용 목록).

핵심 이정표 (0001 ~ 0184 구간 **발췌** — 본 표에 없는 옛 번호·롤백 쌍 파일(0185b 등)·`_cleanup_*` 파일은 `supabase/migrations/` 폴더 참조):
...
| 0280 | top_cards 통계 RPC 사이트 전체 게이트 완화 — `get_top_cards_by_{views\|likes\|saves\|shares\|comments}` 의 사이트 전체(both NULL) 호출을 `is_admin()` 전용 → 로그인 회원 누구나(발행 카드의 조회·좋아요·저장·공유·댓글 수는 피드에 이미 공개되는 정보). 원장(p_doctor_id)·작성자(p_author_profile_id) 필터 경로의 `_check_doctor_kpi_access` 권한 체크는 유지. 집계 로직(`*_inner`)·시그니처·GRANT(authenticated)·anon REVOKE 불변. (표 누락분 소급 등재 — 파일 헤더 주석·CHANGELOG 2026-06-12 근거) | **적용 완료 (2026-06-12)** |
...
| 0329 (data) | **시술 리포트 앵커 데이터 정정 3건 (원장 승인 2026-07-04)** — ① 앵커 9건(2925·2689·2959·3786·2966·3810·2970·3046·3056) 제목 브랜드부 CP949 오염('???? ??? \|') → 0219 템플릿 재적용 ② 앵커 2525 post_slug rejuran-eye→rejuran-i(tag_dictionary.en SSOT 정합, 부분 유니크 NOT EXISTS 가드) ③ tag_dictionary 에 '덴서티알파팁'(en=density-alpha-tip, 부모 '덴서티' 상속 — 리프팅) 재등록(0317/0318 에서 삭제·후기 '덴서티' 이관 이력 — 재등록으로 향후 후기부터 정상 연결). 전문 멱등(재실행 0행). 디비전문가 검수 통과(멱등·스키마·단일매칭·'?' 오폭 위험 전수 실측 — 정당한 '?' 제목 0건). 적용 후 검증: 오염 제목 9→0, 앵커-사전 매칭 63→65/65, U+FFFD 0. UTF-8 경로 적용. |
| 0330 (feed) | **feed/search_cards_scored 의 procedure_review jsonb 에 downtime 추가** — 시술후기 카드 컴팩트 요약(원장 승인 표기안 2026-07-04)의 "회복" 세그먼트용. 각 함수 jsonb_build_object 한 줄만 추가(0327/라이브 본문 VERBATIM), 시그니처 불변 → CREATE OR REPLACE(ACL 보존). PostgREST 임베드 경로는 card-select.ts 두 SELECT 에 동시 추가(같은 commit). 적용 후 실측: 두 RPC downtime 포함·U+FFFD 0·실데이터 흐름(week_1) 확인. tag_cards_scored 는 대상 외. |
| 0331 (security) | **[치명 C-1] current_active_profile_id() 헤더 위조 차단** — 함수가 요청 헤더 `x-active-profile-id` 를 UUID 형식만 검증하고 소유권 확인 없이 반환 → `COALESCE(current_active_profile_id(), auth.uid())` 를 쓰는 **31개 RLS 정책**(cards/comments/diaries/notifications/card_likes/card_saves/comment_likes/push_subscriptions/notification_preferences/diary_procedures/scheduled_notification)이 헤더 위조에 노출(로그인 공격자가 raw REST 로 피해자 시술일기·알림 열람, 타인 명의 댓글·좋아요, 타인 글 수정·삭제 = IDOR). `is_admin()`/`current_doctor_id()` 와 동일한 묶음 게이트(`EXISTS ... id=v_active AND (p.id=v_uid OR p.auth_user_id=v_uid)`) 추가 — 본인 묶음일 때만 반환, 위조면 NULL(호출측 COALESCE 가 auth.uid()=본인 base 로 폴백). 함수 1곳 수정으로 31개 정책 동시 방어. STABLE·SECURITY DEFINER·search_path 유지, PK+auth_user_id 인덱스로 O(1)(성능 무회귀). 정상 사용자(base 176/sub 10) 전부 EXISTS 통과 확인. UTF-8 경로·U+FFFD 0. |
| 0332 (security) | **[높음 H-2] 탈퇴 익명화 묶음 전체 복원** — anonymize_user_content_before_delete() 가 0162 이후 active 명함 1개만 익명화 → 묶음(auth_user_id 공유 9개, 최대 3명) 내 나머지 명함 PII 영구 잔존(deleteUser 의 FK ON DELETE SET NULL 로 auth 링크만 끊김, PIPA 소지). 0109 원설계처럼 묶음 전체(`id=auth.uid OR auth_user_id=auth.uid`) 루프 익명화로 복원. 보완 3: **(1) 의사 명함 제외**(`NOT (role='doctor' OR doctor_id IS NOT NULL)` — 원장 결정 2026-07-04, 의사 실명은 공개 Q&A 작성자로 유지) **(2) v_mask 를 루프 안 r.id 별 생성**(루프 밖 단일 mask 면 묶음 다수가 동일 handle → `idx_profiles_handle_unique` 위반 → 탈퇴 롤백) **(3) fitzpatrick(0325 PII 분류) 도 NULL 스크럽**(코드검수관 지적). `deleted_at IS NULL` 로 재시도 멱등. 옛 forbidden RAISE 제거(스코프 자체가 본인 묶음 한정). **0331 과 반드시 동시 배포**(단독이면 재시도 소유검증 교착). UTF-8 경로·U+FFFD 0. |
| 0333 (security) | **자잘 보안 2건** — ① `card_likes_select` 전체공개(qual=true, anon 포함 좋아요 그래프 열거 가능) → `card_saves_select` 와 동일 `TO authenticated`+본인+admin 패턴으로 좁힘. 소비 3경로 무영향(좋아요여부=.eq(profile_id) 본인 / 좋아요수=cards.like_count denorm+트리거 / 좋아요목록=get_recent_card_likers_batch SECURITY DEFINER). ② `site_visits_anon_insert` with_check(true, anon 임의 방문 위조로 KPI 오염) → 삽입 profile_id 가 호출자 본인 묶음일 때만 허용(`EXISTS profiles ... p.id=profile_id AND (p.id=auth.uid OR p.auth_user_id=auth.uid)`). 정상 적재(middleware, 로그인 사용자, event.waitUntil fail-safe)는 profile_id=본인 명함이라 통과, anon·타인위조 차단. |
| 0334 (security) | **[H-1 단계1] PII 안전 조회 RPC 2종** — authenticated 롤이 타인 profiles PII 를 raw REST 로 수집하던 것을 막기 위한 단계1(REVOKE 前 조회 이관). `get_onboarding_gate(p_target)`→본인 묶음일 때만 (birthdate, terms_agreed_at)(온보딩 게이트용, 경량). `get_profile_pii(p_target)`→본인/is_admin=전체 PII, 타인=field_visibility 필터(`COALESCE((v→>key)::bool, true)` = 기본 공개·명시적 false 만 숨김, ProfileView `v[key]!==false` 규약 일치), contact_email·fitzpatrick 은 타인 항상 NULL. 둘 다 SECURITY DEFINER·STABLE·search_path 고정·`GRANT EXECUTE TO authenticated`(anon REVOKE). 소비처 10곳 이관(identity-server·middleware·onboarding·my·today·[handle]·admin/users·auth/callback·signup). **단계2(authenticated PII 컬럼 REVOKE)는 프로덕션 검증 후 별도.** UTF-8 경로·U+FFFD 0. |
| 0335 (security) | **[H-1 단계2] authenticated PII 8컬럼 SELECT REVOKE (H-1 실차단)** — profiles 테이블 레벨 SELECT 전체 회수 후 안전 23컬럼만 컬럼 레벨 재부여. authenticated 는 birthdate·contact_email·gender·face_shape·skin_type·skin_concerns·interested_procedures·fitzpatrick 직접 SELECT 불가(42501). ⚠ `REVOKE SELECT ON table` 이 컬럼 그랜트까지 전부 회수하므로(PostgreSQL) "전체 회수 + 안전23 재부여" 패턴 필수(테이블+PII컬럼만 회수 시 안전컬럼도 막힘 — 초기 시도 후 정정). 전제: 0334 조회 이관 배포·검증. 실측: PII 42501 차단·안전23 정상·anon 16 무변경·get_profile_pii RPC 작동. UTF-8 경로·U+FFFD 0. |
| 0336 (security) | **예약 핸들 확장** — `reserved_handles` 에 최상위 라우트 및 시스템 용어 다수 추가(reports/topics/today/review/reviews/shop/my/notes/weather/report/rss/onboarding/notifications/debug/cards/doctor/corrections/disclaimer/disclosures/editorial-policy/medical-review/doctor-guidelines/reports-new/app). `check_handle_not_reserved` 트리거·`route-class.ts::RESERVED_FIRST_SEGMENT` 와 동반 갱신. 멱등: ON CONFLICT DO NOTHING. |
| 0337 (cards) | **[R1-1] `cards.category` 잘못된 DEFAULT 제거** — 컬럼 기본값이 폐기 카테고리 `diary` 로 설정돼 자기 CHECK 제약(`qa/doodle/review/review_summary` 4종)을 위반하던 결함 교정. 모든 INSERT 경로(api/articles POST · admin draft publish · 후기/방문 RPC)는 category 를 명시하므로 회귀 0. DROP DEFAULT 선택 이유: SET DEFAULT `doodle` 으로 묻어두면 미래 누락 INSERT 가 조용히 오분류 → 즉시 23502(NOT NULL) 에러로 드러나는 DROP DEFAULT 가 안전. CHECK 제약 무손상 확인. |
| 0338 (tags) | **[R1-3 M-2] `tag_normalization` 역방향 8행 정정** — `canonical`이 `tag_dictionary.ko`(대표어)인 레거시 행이 8개 존재하여 정상 태그 입력 시 `normalizeTag()`가 다른 태그로 재작성하는 오염. 처분: 뒤집기 2행(고압산소치료/마리오네트주름 — variants가 비사전어 → (canonical=비사전어, variants=[대표어])로 정방향 교정) + 삭제 6행(리쥬란HB/리프팅시술/콜라겐자극/덴서티알파팁 — 대표어↔대표어 충돌 / 보톡스내성/보톡스주기 — 분해형). 재발 방지 가드: `gen-tag-dictionary.mjs` prebuild 단계에서 충돌 목록 출력 + `process.exit(1)`로 빌드 차단. 가드 실행 중 8번째 행(덴서티알파팁) 추가 발견·처리. 스냅샷 재생성: norm 191→183건. UTF-8 경로·U+FFFD 0. 오염 감사(읽기 전용): 역방향 행 variants가 포함된 published cards — 리쥬란HB행 94건·리프팅시술행 70건·콜라겐자극행 102건·덴서티알파팁행 18건·보톡스내성행 7건·보톡스주기행 2건(고압산소치료·마리오네트주름행 각 0건). **카드 keywords 교정 여부는 디렉터·원장 판단(데이터 수정 미실행).** |
| 0339 (maintenance) | **[R2-4] `api_rate_limits` opportunistic purge** -- `check_and_increment_rate_limit()` 에 확률 게이트(random() < 0.02, 약 1/50 호출) 방식 기회적 purge 추가. 보존 컷오프 = now() - 2 days (최대 window 86,400s = 24h 의 2배). 기존 `api_rate_limits_window_idx` 인덱스 스캔 DELETE. 시그니처·SECURITY DEFINER·owner·ACL(PUBLIC EXECUTE) 전부 불변(CREATE OR REPLACE). 실측: 적용 시점 1,848행 중 1,729행(93.6%)이 컷오프 대상 -- 자연 정리에 맡김(1회성 즉시 DELETE 미포함). |
| 0340 (stats) | **[QA 이슈 C] `card_impressions` 명함(profile) 단위 집계 정합화** — `profile_id` FK(`card_impressions_user_id_fkey`)가 `auth.users(id)` 를 참조(0048 qa_impressions 원형·0186 리네임 시 방치)해, 클라이언트가 넣는 활성 명함 `profiles.id` 가 비-base(id≠auth_user_id)면 23503 FK 위반으로 노출 INSERT 전량 실패(의사 7·회원 2·관리자 1=10명 대상). 형제 테이블 `site_visits`(profile_id→profiles) 와 동일 구조로: ① 잘못된 FK 제거 ② `card_impressions_profile_id_fkey`(→`profiles(id)` ON DELETE SET NULL) 재지정 ③ 세션 유니크(`card_id, session_id`) 제거 — 명함 단위 dedup 은 클라 impKey 담당(site_visits 철학). 사전조건: non-null profile_id 8,299건 전부 profiles 존재(orphan 0). BEGIN…ROLLBACK 검증 + 비-base 명함 INSERT 201 시뮬레이션 통과. 클라(`impression-queue.ts` 단순 insert·`useCardViewer.ts` impKey 명함 포함)와 동반. |
| 0341 (clinic) | **병원 계정 · 시술노트 대행 Part A — 원장 3토글 모델** — `doctors` 3컬럼 신설: `clinic_id`(bigint FK→`clinics` ON DELETE SET NULL, 건보 심평원 지점 코드 참조·불변) · `is_affiliated`(bool DEFAULT true, 재직/소속 — 퇴사 시 false → 드롭다운 제외) · `is_listed`(bool DEFAULT true, 공개 페이지 on/off — 퇴사와 독립). `doctors_clinic_id_idx` 부분 인덱스(clinic_id IS NOT NULL). 기존 원장 9명 `branch`('강남점 등') → `clinic_id` backfill(강남 16957·건대 16956·대구 16958·수원 16959·판교 16955, 주소 교차확인). `admin_create_doctor_profile` 6→8인자(DROP 후 재생성, `p_clinic_id bigint`·`p_is_listed boolean DEFAULT false`=신규 원장 비공개 기본) — 본문·검증·묶음중복·slug중복·handle생성·profiles INSERT·RETURN 은 0192 VERBATIM, doctors INSERT 절에만 신규 2컬럼 추가. service_role EXECUTE 전용. 전부 additive·비파괴. |
| 0342 (clinic) | **Part B — `profiles` clinic 역할·소속·실명 (★enum 2-call 적용)** — `user_role` enum 에 `clinic` 추가(ALTER TYPE ADD VALUE 는 트랜잭션 밖 단독 POST 후 커밋 확인 → 별 트랜잭션에서 나머지 DDL, 0152 선례). `profiles.clinic_id`(bigint FK→`clinics` ON DELETE SET NULL, `doctor_id` 와 대칭) · `profiles.legal_name`(text, 선택 복원용 실명 — 쇼핑몰 주문·결제 대비·회원 미입력 허용, PII, CHECK 1~50자, anon/authenticated REVOKE ALL). `is_clinic(p_clinic_id bigint)`(SECURITY DEFINER STABLE — active 명함이 role='clinic'·clinic_id 일치·`auth.uid()` 소유일 때만 true, 묶음 위조 차단, authenticated EXECUTE). `reserved_handles` 에 'clinic' 추가(ON CONFLICT DO NOTHING). |
| 0343 (clinic) | **Part B — `diaries` 병원 대행 5컬럼** — 전부 additive: `source`(text NOT NULL DEFAULT 'member' CHECK member/clinic — 작성 주체, 병원 대행분=clinic) · `created_by_clinic_profile_id`(uuid FK→profiles ON DELETE SET NULL — 대행 입력한 병원 계정 감사) · `linked_consent_at`(timestamptz — 연결 동의 시각 스냅샷) · `next_appointment_date`(date — 재방문 리마인더) · `doctor_id`(uuid FK→doctors ON DELETE SET NULL — 병원 모드 담당 원장, source='clinic' 전용). ⚠ `diaries.source`(member/clinic=작성 주체) ≠ `procedure_reviews.source`(standalone/diary_linked=후기 파생) **동명·다른 도메인**. RLS 변경 없음(병원의 회원 소유 diaries 생성은 SECURITY DEFINER RPC 0345 전용, 후속). |
| 0344 (clinic) | **Part B — `clinic_member_links` 테이블 신설** — 병원(지점 계정)↔회원 명함 연결(비귀속 다대다) + 병원 보유 환자 기록(동의 시 profiles 에서 1회 복사한 스냅샷 + 병원 자체 입력). 컬럼: `id` bigserial PK / `clinic_id` FK→clinics CASCADE / `profile_id` FK→profiles CASCADE(회원 명함) / `status`(pending/active/rejected/revoked) / `created_by_clinic_profile_id` FK→profiles(등록 병원 계정) / `consent_at`·`consent_version` / 병원 입력 대조·감사용 `requested_legal_name`·`requested_birthdate` / `registration_number`(병원 내부 챠트번호·고유값 아님) / 병원 직접입력 `patient_phone`·`patient_address` / 회원→병원 스냅샷 `patient_name`·`patient_birthdate`·`patient_email`·`patient_skin_profile`(jsonb) / `created_at`·`revoked_at`. 인덱스 2(clinic_id·profile_id) + 부분 UNIQUE(`clinic_id, profile_id`) WHERE status IN(pending,active) — 병원↔회원 쌍당 미처리/유효 연결 최대 1건(rejected/revoked 는 재요청 가능). **RLS ENABLE + anon/authenticated REVOKE ALL(정책 0개 = 직접 접근 완전 차단, SECURITY DEFINER RPC 0345 전용, 후속)**. |
| 0345 (clinic) | **Part B — 병원 대행 RPC 9종 + `is_notification_enabled` clinic 확장** (2026-07-06 적용 — 0346·0348 보다 늦게 작성·적용, 번호는 계획 예약분) — 전부 SECURITY DEFINER + `search_path 'public','pg_temp'` + 호출자 명함 파라미터+`auth.uid()` 대조(0279 계승, GUC 비의존). 병원측: `clinic_request_link`(handle+생일 하드대조 — 실패 사유 비구분 열거방지·지점단위 rate-limit 10/분·pending 생성·§8.3 문구 알림·23505 race 핸들러) / `clinic_add_visit`(active 연결 필수 FOR UPDATE·자기지점 재직 원장만·diaries source='clinic'+0343 5컬럼·clinic_name=병원 명함 display_name·price 상한 20억·도착 알림) / `clinic_update_patient`(전체 교체·requested_* 불변·pending 수정 허용) / `get_clinic_patients`(ILIKE 와일드카드 이스케이프)·`get_clinic_patient`(requested_* 반환 금지). 회원측: `member_respond_link`(동의 시 스냅샷 1회 복사·consent_version 서버상수 'v1-260706'·실명 선택 백필) / `member_get_clinic_link`·`member_list_clinic_links`(REVOKE ALL 테이블의 유일한 회원 조회 경로 — 동의 화면·연결관리용, 본인 링크만) / `member_revoke_clinic_link`(active→revoked). **계획 §6.7 `member_delete_clinic_visit` 은 미생성** — 기존 `delete_visit`(0297)이 후기 standalone 전환+예약 cancel 까지 완전 처리, source='clinic'(소유=회원)에도 그대로 적용(중복 금지). `is_notification_enabled` 에 clinic 2종 분기 추가(+search_path pg_temp 강화). 디비전문가 2차 검수 통과(치명 4건 반영: FOR UPDATE·배열검증·price 상한·unit_text 길이). U+FFFD 0 재스캔. |
| 0346 (clinic) | **Part B — clinic 알림 종류 2종 + 회원 on/off** — `notifications.kind`(text+CHECK) 10→12값 추가: `clinic_link_request`(병원이 연결 요청) · `clinic_visit_added`(병원이 새 시술노트 대행 작성). `notification_preferences` 에 `pref_clinic_link_request`·`pref_clinic_visit_added`(bool DEFAULT true=켜짐). 병원명 비노출은 알림 표시 코드(`notification-kinds.ts`·push send)에서 처리 — 여기선 종류만 등록. CHECK 는 enum 아님 → DROP+ADD 트랜잭션 안전. |
| 0347 (clinic) | **탈퇴 익명화에 clinic PII 파기 추가** — `anonymize_user_content_before_delete()` 에 (1) profiles UPDATE SET 절에 `legal_name=NULL`(0342 신설) (2) 회원 명함 루프 안에서 그 명함에 걸린 `clinic_member_links`(WHERE profile_id=r.id) 회원 유래 PII 8필드(patient_name/email/birthdate/skin_profile·patient_phone·patient_address·requested_legal_name·requested_birthdate) NULL 처리 추가. 0332 본문(시그니처·의사 명함 제외·묶음 루프·멱등 필터) 100% 보존. **`registration_number`(병원 내부 챠트번호)는 파기 대상 제외·존치** — 개인 단독 식별자 아님(고유값 아님) + 병원 진료기록 보존 의무 대상이라 탈퇴로 병원 챠트번호까지 지우면 병원 측 기록 정합 붕괴. |
| 0349 (clinic) | **병원 대시보드 집계 RPC** — `get_clinic_dashboard(p_clinic_profile_id uuid)` RETURNS TABLE(patient_total·pending_count·active_count·notes_today·notes_month). SECURITY DEFINER STABLE + 병원 명함 검증(auth.uid 대조, 0345 패턴). clinic_member_links 상태별 count + diaries(source='clinic'·clinic_id) 오늘/이번달(KST `AT TIME ZONE 'Asia/Seoul'`) count. diaries 는 회원 소유 RLS 라 병원 직접 SELECT 불가 → 이 RPC(owner)만 집계. authenticated EXECUTE. |
| 0350 (clinic) | **병원 시술기록 관리 RPC 5종 + 인덱스 2종** (운영 프로그램 S1, 계획 `260706 …관리 설계.md`) — `get_clinic_patient_visits`(환자 1명 타임라인+procedures jsonb, **3중 경계** source='clinic'·clinic_id=자기·profile_id=연결회원, status 무관 조회) · `get_clinic_visits`(지점 전체 대장, visited_on 범위 p_from~p_to·검색·원장필터·정렬 CASE 화이트리스트·COLLATE ko-x-icu·limit/offset) · `get_clinic_calendar_summary`(월 날짜별 count, 범위비교로 인덱스 활용) · `clinic_update_visit`(3중+active만[C2]+후기시 차단[C5]+**불변컬럼 미SET**+diary_procedures 전체교체, 알림 미발송[C13]) · `clinic_delete_visit`(3중+active만[C2]+후기시 차단[C5]+scheduled_notification 정리). 전부 SECURITY DEFINER·search_path 고정·REVOKE/GRANT(0345 패턴). 인덱스: `diaries_clinic_visited_idx (clinic_id, visited_on DESC) WHERE source='clinic'` · `clinic_member_links_clinic_birthdate_idx (clinic_id, patient_birthdate)`. 3인 설계검토+디비전문가 독립검수(치명 0). |
| 0353 (clinic) | **`update_visit` 회원 편집 안전화 + 시술목록 편집** (C4, §4.2 치명2) — 회원 시술노트 편집 UI 신설의 선행 정정. (16→17인자 DROP 후 재생성.) [A] **source='clinic' 노트 보존**: 회원이 병원 대행 노트를 편집해도 `clinic_id`·병원 위치 스냅샷(clinic_name/addr/tel/x/y/home/kakao)을 기존값 CASE 보존(0297 이 `clinic_id=p_clinic_id` 로 덮어써 병원 귀속 훼손하던 사고 차단). source/created_by/linked_consent_at 은 SET 절 밖이라 이미 불변. [B] **`p_procedures jsonb DEFAULT NULL` 추가**: 지정 시 diary_procedures 전체 교체(후기 있으면 `visit_has_linked_reviews` 차단 — diary_procedure_id SET NULL 소실 방지, clinic_update_visit 대칭), NULL 이면 시술 불변(하위호환 — 기존 /api/visits/[id] PATCH 16인자 호출 무영향). 디비전문가 독립검수 치명 0. |
| 0352 (clinic) | **`get_clinic_patients` 검색 OR 정정** — 0351 이 `p_birthdate` 를 별도 AND 필터로 걸어 "텍스트 검색 AND 생일" 동시 만족을 요구하던 것을, 한 검색창에서 이름·등록번호·아이디·생일 중 **아무거나 OR 매칭**하도록 WHERE 재구성(원장 요구 §2.3). 클라는 검색어 원문을 p_search 로, 그 원문이 완전한 생일(parseFreeBirthdate)이면 p_birthdate 로도 함께 전달. 시그니처·반환 불변 → CREATE OR REPLACE(DROP 불필요). 나머지(명함검증·정렬·집계·COLLATE)는 0351 그대로. |
| 0351 (clinic) | **`get_clinic_patients` v2** — 반환컬럼 추가(last_visit_on·visit_count·age_years)는 CREATE OR REPLACE 불가 → **`DROP FUNCTION (uuid,text)` 후 재생성**. 신규 파라미터(p_birthdate date·p_status_filter·p_sort_by·p_sort_dir·p_limit·p_offset). 검색=이름·등록번호·핸들 ILIKE + **생일 동등비교**(클라 파싱 date, to_char ILIKE 폐기). 집계 LATERAL 에 `clinic_id=자기` 필수(타 병원 노트 혼입 방지). 정렬 CASE 화이트리스트+ko-x-icu. 기존 13컬럼 순서 보존(하위호환 — 기존 프론트 호출 무영향). |
| 0348 (clinic·security) | **[치명 2건 정정] 신규 컬럼 GRANT 보강** — 0341/0342 가 신규 컬럼을 추가하며 0335(profiles authenticated 컬럼단 화이트리스트 23컬럼)·0190(doctors service_role UPDATE profile_data 한정) GRANT 를 확장 안 해 42501 발생. **[C1]** `resolveActiveIdentity` 가 `profiles.clinic_id` SELECT → 권한 없음 → data=null → 전 로그인 유저 401(사이트 마비): `GRANT SELECT(clinic_id) profiles→authenticated`(비-PII 지점 코드라 안전, `legal_name` 은 PII 라 미부여·차단 유지). **[C2]** admin settings 라우트가 service_role 로 `doctors.clinic_id/is_affiliated/is_listed/branch/slug` UPDATE → 42501: `GRANT UPDATE(...) doctors→service_role`. 부수: 0347 `GRANT EXECUTE` 재명시(관례 통일), 0344 `clinic_member_links_id_seq` 방어적 REVOKE. 운영 DB 검증 후 정정. |
| 0354 (clinic) | **`create_procedure_review` 노트↔후기 연결(visit_id/diary_procedure_id)** — 19인자 DROP 후 `p_visit_id bigint`·`p_diary_procedure_id bigint`(끝, DEFAULT NULL) 추가 재생성(오버로드 not-unique 회피). visit_id 전달 시 diary 소유(`diaries.profile_id=p_author_id`)·dp 정합(`dp.diary_id=visit_id`) 검증 후 `source='diary_linked'`+visit_id 저장(source_link_chk 통과), 미전달이면 standalone(무회귀·기존 standalone 후기 불변). 본문(cards/procedure_reviews INSERT·short_answers·리포트 앵커) 100% 보존. GRANT EXECUTE authenticated 복원. U+FFFD 0 재스캔. 노트 상세는 diary_procedure_id(FK)로만 '이미 씀' 판정(procedure_ko 텍스트매칭 금지). |
| 0355 (clinic·hardening) | **병원 RPC 소규모 하드닝** — ① 다음 예약일 미래 상한(오타 방어) `> current_date + interval '5 years'` 거부를 `clinic_add_visit`·`clinic_update_visit` 에 추가(기존 '방문일 이전' 검사와 병렬). **회원 `update_visit` 은 예약일 파라미터가 없어 대상 아님**(pg_proc 실측 — 계획의 '세 곳' 정정). ② `admin_create_doctor_profile` `SET search_path` 에 `pg_temp` 추가(shadow 차단, 0345/0350 규약 정합). 세 함수 모두 현재 정의(pg_get_functiondef) 그대로 두고 해당 줄만 수술적 수정(CREATE OR REPLACE). FOR UPDATE 단일잠금은 생략(`ORDER BY … LIMIT 1 FOR UPDATE` 는 반환 1행만 잠금 — 감사 지적 미성립). U+FFFD 0. |
| 0357 (analytics) | **`traffic_landings` 유입 분석 테이블 + `get_traffic_overview(p_days)` RPC** (2026-07-11, 구 리서치 패널 대체). 컬럼: landed_at·landing_path·referrer_host·channel(분류)·utm_*(5)·device·os·in_app·country·region·is_member. **IP 원본 미저장**(국가/지역 코스만 — PIPA 비식별). 인덱스 landed_at·(channel,landed_at). RLS: anon/authenticated INSERT(WITH CHECK true)만, **직접 SELECT 정책·GRANT 없음(RLS+권한 이중 차단)** — 집계는 SECURITY DEFINER RPC 경유. RPC: `is_admin()` 가드 + search_path 고정 + authenticated EXECUTE, jsonb 반환(채널·상위 유입처/랜딩·기기/OS·인앱·캠페인·KST 일별). 수집=클라 비컨(LandingTracker→POST /api/landing, 세션 1회·IP rate-limit). 검색어(오가닉)는 자체 수집 불가 — GSC/네이버 서치어드바이저 전용. U+FFFD 0. |
| 0356 (clinic) | **`get_clinic_patients` 정렬에 registration_number(등록번호) 추가** — 0352 정의 유지 + 정렬 화이트리스트에 `'registration_number'` + ORDER BY CASE 분기 2개(asc/desc, **NULLS LAST** — 등록번호 NULL 다수라 뒤로). 시그니처·반환·검색·집계·COLLATE 등 나머지 로직 불변(CREATE OR REPLACE). 프론트 3중 화이트리스트(RPC / `patients/page.tsx` SORT_BY / `ClinicPatientsView` SORT_OPTIONS) 동시 확장. U+FFFD 0. |
| 0358 (doctors) | **원장 대표 논문 `profile_data.pmids`(PMID만) → `papers`(pmid+title+journal+year) 승격 백필** (2026-07-18). 9명 전원 `(profile_data - 'pmids') \|\| jsonb_build_object('papers', …)` 로 pmids 제거 + papers 병합(다른 profile_data 필드 보존). 제목·저널·연도는 PubMed esummary 정규값. 스키마 변경 아님(JSONB 값 백필). 코드 SSOT: `src/lib/doctor-profile.ts` `DoctorPaper`/`getDoctorPapers` — JSON-LD `ScholarlyArticle` 에 name·datePublished·isPartOf(저널) 추가 + 원장 프로필 "대표 논문" 표시. 부수 교정: 박효진 원장 오류 PMID `37222165`(피부과 아닌 Soft Matter 물리 논문) 제거, 정상 2편으로 교체. |
