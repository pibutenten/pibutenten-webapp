...
- **마이그레이션**: Phase 1 (본 ADR) — 문서·hook 만. Phase 2~4 — DB 컬럼 RENAME 후속 (번호 예약은 본 ADR 의 "마이그 번호 표" 참조).
...
신규 테이블·신규 마이그레이션에서 `user_id` 컬럼 도입 금지. 이미 존재하는 `user_id` 컬럼 9개는 Phase 2~4 에서 `profile_id` 로 RENAME.
...
본 ADR 의 결정은 즉시 발효. 다만 production DB 의 11개 테이블 컬럼 RENAME 은 Phase 2~4 로 분할:

- **Phase 2** — 인터랙션·통계 6 테이블: daily_logins, site_visits, activity_points, card_shares, card_views, card_impressions
- **Phase 3** — 좋아요·저장 3 테이블: card_likes, card_saves, comment_likes
- **Phase 4** — 콘텐츠 2 테이블: **보류 — author_id 유지**. 단 PostgREST embed FK 이름 정합 점검 후 결정 재검토.

본 ADR 작성 시점 (Phase 1) 에서는 **DB 컬럼은 한 글자도 안 바뀐 상태**. 문서·hook 만 정합 완료.
...
| 0186 | Phase 2 — 인터랙션·통계 6 테이블 컬럼 `user_id` → `profile_id` RENAME + FK/index/RLS 갱신 | 예약 |
| 0187 | Phase 3 — 좋아요·저장 3 테이블 RENAME + 트리거·RPC 재정의 | 예약 |
...
- Phase 2~3 마이그레이션 동안 옛 컬럼명 (`user_id`) 과 새 컬럼명 (`profile_id`) 이 production DB 안에 공존. 그 기간 동안 코드·RLS·RPC 가 mixed 상태로 부분 적용되어야 함.
...
- Phase 2 진행 직전 RPC 64개 중 (A)류 40개 재정의 필요 (Phase 0.5 에서 정확 식별 완료).
