...
### 5. settings/profile 은 아직 base-only (POLICY-1 잔여)

- `src/app/settings/profile/page.tsx:57` 의 `.eq("id", user.id)` 는 여전히 base profile 만 읽음.
- 본 ADR 적용 후에도 미정합 상태. **별도 안건으로 정리 예정** — 본 ADR 본문에 거짓 "완료" 표기 없음.
- 정합 대상: PRD/ADR 본문, middleware/onboarding/comments POST, settings/profile (이번 ADR 의 후속).

### 6. 새 sub 명함 생성 시 자동 propagate (현재 미구현 — 정책 보류)

- `src/` 안 `profiles INSERT` 0건 / `create_sub_profile` 류 RPC 0건 확인.
- 정상 sub 명함 생성 경로 현재 코드베이스에 부재 — `/api/admin/users/[id]/role/route.ts` (CRITICAL-3) 만 sub 생성 경로이나 이는 사용자 정책상 별도 안건.
- 향후 정상 sub 생성 경로 도입 시점에 propagate 호출을 같은 commit 에서 추가하는 정책 명시.
