...
- ~~settings/profile (POLICY-1 4번째 게이트) 정합 별도 안건 — 본 ADR 직후 진행 권장.~~ → **해소 완료**: ① 2026-05-29 commit `fd1b64b` 로 active 명함 단위 정합 (§5 — POLICY-1 잔여 0). ② 2026-06-15 신규 스킨 승격으로 `/settings`·`/settings/profile` 은 redirect 경유지화 (편집 UI 는 본인 프로필 아코디언). ③ 2026-07-08 UI 개편 Phase 4 (ADR 0026) 로 설정 전용 라우트 `/my/settings` 정식화 — 데이터 조립은 `src/lib/profile-settings-data.ts::buildProfileSettingsProps` (§5 와 동일한 `getIdentityContext` SSOT `targetProfileId` 단일 결정 유지).
