...
- **(−/회귀→수정) 스크롤 컨테이너 persist** — 실제 스크롤은 `AppShell .root`(overflow-y:auto)에서 일어나는데, 셸이 persist 되며 인덱스의 스크롤 위치가 상세로 남아 "맨 위가 아닌 곳에서 시작"하는 회귀가 생겼습니다(앱의 `ScrollManager` 는 `window` 만 관리 = `.root` 미관여가 근본원인). `ReportsDetailView` 마운트 시 가장 가까운 스크롤 조상(`.root`)을 찾아 `scrollTop=0` 으로 리셋해 해결(공용 `AppShell`/`ScrollManager` 미변경). 단 detail→index 뒤로가기 시 인덱스 스크롤 복원은 당시 미적용이었으나, **2026-06-30 P2 에서 sessionStorage 스냅샷(sort/pageCount/openSet/scrollTop/category, 5분 TTL)으로 복원 해소**(`ReportsIndexView`). `ScrollManager` 의 `.root` 일반화(앱 전반 뒤로가기 스크롤 복원)는 여전히 별도 안건.
