...
- **칩바 = 헤더와 한 덩어리(피드 표준, 2026-07-09)**: 페이지 상단 정렬·필터 칩은 **셸 헤더 칩바 슬롯**(`AppShell chips` → `.topStack`)에 두어 헤더와 한 요소로 함께 이동/숨김(모바일 동시 숨김·데스크탑 상시 고정). 피드(카테고리·태그 칩)·**리포트 허브(정렬 칩)**가 이 방식. 정렬 상태는 공유 셸(ReportsShell)이 소유(ReportsSortContext)해 본문에 내려줌.
  - **본문 sticky 칩 계층 표준**(facc6eb): 헤더에 못 올리는 **섹션 내부** 칩(예: 리포트 상세 후기 정렬 — 페이지 최상단이 히어로라 헤더 부적합)은 본문 sticky 로 두되 셸 헤더 묶음(z-40)보다 **아래 z-30**(복귀 헤더가 위를 덮음) + 모바일 `top=var(--sat)` + 데스크탑 고정 헤더 "아래"(top 72px)에 **자기 구간 동안만** 고정. 헤더를 가리는 계층 금지. 리포트 상세 후기 정렬 칩이 이 방식(섹션 제목은 고정 제외 — 일반 흐름으로 지나가고 정렬 칩만 sticky, 원장 확정 2026-07-09).
...
### 6.2. 유입 분석 (Acquisition, 관리자 대시보드)
- **채널 집계 (자체 수집)**: 방문 세션 첫 진입을 채널(검색/SNS/메신저/직접/앱/기타)·유입처 도메인·랜딩 페이지·기기/OS·인앱 브라우저·UTM 캠페인별로 집계. 클라 비컨(`LandingTracker`, 세션 1회) → `POST /api/landing` → `traffic_landings`(IP 원본 미저장) → `get_traffic_overview` RPC. 네이티브 앱 진입은 별도 "앱" 채널로 분리.
- **유입 검색어 (구글 서치콘솔 연동)**: "무슨 검색어로 찾았는지"는 검색엔진이 referrer 로 안 넘겨 자체 수집 불가 → 구글 Search Analytics API(서비스 계정, 읽기전용, 6h 캐시)로 상위 검색어(클릭/노출/CTR/순위)를 조회해 대시보드에 표시. 자격증명 미설정 시 설정 안내만 노출(graceful). 네이버는 검색어 API 미공개라 미포함.
