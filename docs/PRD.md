...
- **칩바 = 헤더와 한 덩어리(피드 표준, 2026-07-09)**: 페이지 상단 정렬·필터 칩은 **셸 헤더 칩바 슬롯**(`AppShell chips` → `.topStack`)에 두어 헤더와 한 요소로 함께 이동/숨김(모바일 동시 숨김·데스크탑 상시 고정). 피드(카테고리·태그 칩)·**리포트 허브(정렬 칩)**가 이 방식. 정렬 상태는 공유 셸(ReportsShell)이 소유(ReportsSortContext)해 본문에 내려줌.
  - **본문 sticky 칩 계층 표준**(facc6eb): 헤더에 못 올리는 **섹션 내부** 칩(예: 리포트 상세 후기 정렬 — 페이지 최상단이 히어로라 헤더 부적합)은 본문 sticky 로 두되 셸 헤더 묶음(z-40)보다 **아래 z-30**(복귀 헤더가 위를 덮음) + 모바일 `top=var(--sat)` + 데스크탑 고정 헤더 "아래"(top 72px)에 **자기 구간 동안만** 고정. 헤더를 가리는 계층 금지. 리포트 상세는 '후기 헤더+정렬칩'을 한 sticky 블록으로 묶어 적용.
