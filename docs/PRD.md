...
- **검색 결과 블렌딩**: 결과는 피드 + 시술 리포트 블렌딩 — 검색어가 시술명과 매칭되면 '전체' 탭 **최상단에 시술 리포트 1장**(`getProcedureReport`). `review_summary` 카드는 **홈 피드에 직접 주입하지 않고**(비검색 홈 풀은 `feed_cards_scored`), 검색 블렌딩으로만 노출.
...
- **시술 리포트** `/reports/[procedure]`: `procedure_reviews` 를 **실시간 집계**(저장 카드 없음 → 후기 추가 시 자동 반영)한 단일 카드. 만족도(분포)·통증·재시술 의향·체감 효과 + 작성자 성별·연령(집계 RPC, 개별 PII 비노출). index + JSON-LD `MedicalWebPage` + `Service`(additionalType=`MedicalProcedure`) + `AggregateRating`(만족도)·재시술%·통증 + `BreadcrumbList` (의료 시술이라 `Product` 폐기 2026-06-05). 시술명 검색(인-헤더 통합 검색, 홈 `/?q=`, §4.2) 결과 **최상단** 노출. 정식 URL=`/reports/{ko}`(한글, 영문은 308 전용). **`/topics`(전문의 Q&A 허브)와는 분리** — 자기잠식 방지로 /topics 에 리포트 카드 미노출, 양쪽 얇은 링크만(2026-06-05).
- **리포트 허브** `/reports`: N≥4 게이트를 통과한 시술 리포트를 후기 수 내림차순으로 나열(리포트 탭 진입점, `force-dynamic`, 자격 0건이면 noindex). 신디자인 정식 적용 — 인덱스·상세 모두 새 디자인(2단 레이아웃 + 회전 헤드라인), 상세 후기 카드에 작성자 나이·성별 표시. SEO 셸(JSON-LD·canonical·en→ko 308)은 보존(렌더만 교체). 구 미리보기 `/reports-new` 는 `/reports` 로 308 영구 리다이렉트.
