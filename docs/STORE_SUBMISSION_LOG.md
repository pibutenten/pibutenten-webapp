...
> ✅ **1.0.1 양대 스토어 제출 완료(2026-06-29)**: 피부날씨 위치 동(洞) 단위 측위(COARSE) 수정본을 **iOS·Android 모두 새 빌드로 심사 제출 완료**. iOS = App Store Connect 1.0.1(빌드 8) **심사 대기 중**(submission `79b0bab6-cbdd-43b2-bf43-25f167705936`, 148개국). Android = Play 프로덕션 1.0.1(versionCode 2) AAB 업로드 + **전세계 177개국 확대**까지 묶어 **검토를 위해 변경사항 3개 전송 완료**. AAB 는 GitHub Actions `Android Release (AAB)` 수동 dispatch 로 빌드(run 28360296223, 서명 완료 4.58MB, `builds/pibutenten-1.0.1-vc2.aab`). 정밀도는 동 단위로 충분 → FINE 미사용(COARSE 유지, ADR 0022). 양쪽 모두 Google/Apple 검토 승인 대기 중. (상세 §2 2026-06-29)
...
| 스토어 | 버전 | 상태 | 제출일 | 승인 후 |
|---|---|---|---|---|
| **iOS (App Store)** | **1.0.1 (빌드 8, iPhone 전용)** | **심사 대기 중 (Waiting for Review)** — GPS 측위 수정본, 148개국 | 2026-06-29 | 승인 시 자동 출시(자동 버전 출시 ON) |
| **Android (Google Play)** | **1.0.1 (versionCode 2)** | **검토 중 (변경사항 3개 전송 완료)** — 1.0.1 새 버전 + 전세계 177개국 확대 | 2026-06-29 | 관리형 게시 OFF → 승인 시 자동 게시 |

**→ iOS: 1.0.1(빌드 8) 심사 제출 완료, "심사 대기 중". GPS 기반 피부날씨 측위 수정. 자동 버전 출시 ON → 승인 즉시 출시. (대기)**
**→ Android: 1.0.1(versionCode 2) AAB 업로드 + 국가 1→177개국 확대를 묶어 검토 전송 완료. 사전 검사 통과 후 Google 검토 큐 전달. (대기)**
**※ 직전 출시본**: iOS 1.0(빌드 7)·Android 1(1.0)은 2026-06-19 제출 → 6/21~6/23 승인·게시 완료(대한민국). 1.0.1 승인 시 대체됨.
...
### 2026-06-29 — 1.0.1 양대 스토어 재심사 제출 (GPS 측위 수정 · Android 전세계 확대)
- **배경**: 피부날씨 위치가 동(洞) 단위로 표시되도록 기기 측위(COARSE) 수정한 ADR 0022 변경분을 출시본에 반영하려면 **새 빌드 + 스토어 재심사** 필요(웹 배포만으로는 네이티브 미반영). iOS·Android 모두 1.0.1 새 빌드로 재심사 제출.
- **iOS (App Store Connect) — 심사 제출 완료**:
  - 1.0.1 **빌드 8**(iPhone 전용) 첨부, 자동 버전 출시 ON.
  - 상태 **"심사 대기 중(Waiting for Review)"**. submission `79b0bab6-cbdd-43b2-bf43-25f167705936`, 제출 2026-06-29 17:44(배정민), 148개국.
  - ※ App Store Connect 자동화(MCP)로 제출 가능 → 어시스턴트가 직접 처리.
- **Android (Google Play) — 검토 전송 완료**:
  - **AAB 빌드**: GitHub Actions `Android Release (AAB)` 수동 dispatch(run **28360296223**, success) → 서명 완료 AAB 4.58MB 산출 → `builds/pibutenten-1.0.1-vc2.aab` 로 다운로드·검증(유효 번들). applicationId `kr.pibutenten.app`, versionCode 2, versionName 1.0.1.
    - ※ 직전 라이브러리에는 1.0(6/17, versionCode 1) AAB 1개뿐 → "App Bundle 없음" → 새 1.0.1 AAB 를 별도 빌드해 업로드해야 했음(해결).
  - **업로드**: Play 프로덕션 트랙에 1.0.1 AAB 업로드(버전 코드 2 인식, 서명/버전 충돌 없음). 비차단 경고 3건(기기 6종 지원 제외 ~0% / mapping 파일 미첨부 / 네이티브 디버그 기호 미업로드) — 모두 경고, 오류 0건.
  - **전세계 확대**: 배포 국가 **대한민국 1개국 → 177개국**(175개국 추가 + 기타 국가 추가).
  - **검토 전송**: 게시 개요에서 ① 1.0.1 전체 출시(100%) ② 175개국 추가 ③ 기타 국가 추가 = **변경사항 3개**를 묶어 **"검토를 위해 변경사항 3개 전송 완료"**. 사전 검사 통과 후 Google 검토 큐 전달. 관리형 게시 OFF → 승인 시 자동 게시.
  - 출시 노트(ko-KR): "위치(GPS) 기반 피부날씨 기능의 오류를 수정하고 안정성을 개선했습니다."
  - ※ Play Console 은 Chrome 확장 자동화 정책 차단 → 빌드(어시스턴트, gh CLI)와 업로드·게시(Playwright 세션, 원장 로그인) 분담.
- **결과**: iOS·Android 양쪽 모두 1.0.1 새 빌드로 Google/Apple 검토 승인 대기. 정밀도는 동 단위로 충분 → FINE 미사용(COARSE 유지, ADR 0022).
...
### 조치 필요 (현재)
- iOS: **1.0.1(빌드 8) 심사 제출 완료, "심사 대기 중"**. 자동 버전 출시 ON → 승인 즉시 출시. 잔여 조치 없음(반려 시 사유 이메일 대응).
- Android: **1.0.1(versionCode 2) + 177개국 확대 검토 전송 완료**. 관리형 게시 OFF → 승인 시 자동 게시. 잔여 조치 없음.
- **→ 양쪽 스토어 1.0.1 재심사 검토 대기. 차단 조치 없음. (직전 1.0 출시본은 라이브 유지, 1.0.1 승인 시 대체)**
