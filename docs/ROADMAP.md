...
### 시술 리포트 — 상세 로딩속도 최적화 (잔여)
- 신디자인 정식 `/reports` 승격은 완료(ADR 0024/0025, CHANGELOG 2026-06-29). 잔여 과제: 리포트 상세의 `force-dynamic` + 집계 RPC 절대 시간(≈0.8s)을 **ISR/쿼리 최적화**로 줄이는 별도 안건. (공유 셸 persist 로 전환 체감만 개선됐을 뿐 근본 로딩속도는 미해결.)
...
### 피부 날씨 UI 재설계 — 대부분 완료, 2건 보류 (2026-06-17 세션)
- [x] 색 통일: 값 비례 폐기 → **등급 직결 신호등**(좋음 초록 #2BB36B / 보통 노랑 #FFD23A / 나쁨·높음 빨강 #EE4444 / 매우나쁨·매우높음·위험 진빨강 #6E0712). 배경(fill)·글씨(text) 분리(`TIERS`)로 노랑 가독 확보. `weather-logic.ts`.
- [x] 라벨 "UVB 홍반"→"UVB 태닝" 전면. 강수확률 표시 제거(요약 하단·주간 💧). 주간 박스 배경 회색 회귀 fix.
- [x] 상세 KPI 게이지 세로→**가로 바** 전환. today 두 카드 여백은 이미 22px 통일(변경 불필요). 온도 새 버전은 현행 유지.
- [ ] **(보류) flat 일러스트 교체**: `WeatherIllustration.tsx` 날씨 SVG 10종(맑음/구름/비/소나기/폭우/눈/천둥/안개/폭염/한파)을 더 단순한 flat 아이콘으로. 분량 커서 한 종씩 다듬기 필요.
- [ ] **(보류) 날씨 카드 로딩 속도**: `SkinWeatherCard`/`useWeather` 가 client 전용 + geolocation(데스크탑 느림·4s 타임아웃) + 외부 API(Open-Meteo·CAMS) + sessionStorage(세션 한정) 캐시라 첫 진입 스켈레톤 지연. 개선안: ①캐시 sessionStorage→**localStorage**(재방문 즉시 seed) ②첫 진입 시 기본위치(대치동) 즉시 렌더 후 측위 교체 ③권한 거부 데스크탑은 측위 스킵.
